...
/** Golden WU app-on-global-http-error -- onGlobalHttpError. */
describe('AppComponent onGlobalHttpError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onGlobalHttpError({} as any); }).not.toThrow();
  });
});
