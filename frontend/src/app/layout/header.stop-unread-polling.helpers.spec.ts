...
/** Golden WU header-stop-unread-polling -- stopUnreadPolling. */
describe('HeaderComponent stopUnreadPolling (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).stopUnreadPolling(); }).not.toThrow();
  });
});
