...
/** Golden WU header-start-unread-polling -- startUnreadPolling. */
describe('HeaderComponent startUnreadPolling (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).startUnreadPolling(); }).not.toThrow();
  });
});
