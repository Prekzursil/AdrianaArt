...
/** Golden WU header-toggle-notifications -- toggleNotifications. */
describe('HeaderComponent toggleNotifications (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toggleNotifications(); }).not.toThrow();
  });
});
