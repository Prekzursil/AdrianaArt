...
/** Golden WU footer-on-document-click -- onDocumentClick. */
describe('FooterComponent onDocumentClick (golden WU)', () => {
  it('returns early on guard', () => {
    ...
    expect(() => (cmp as any).onDocumentClick({ target: null } as any)).not.toThrow();
  });
});
