...
/** Golden WU header-open-notification -- openNotification. */
describe('HeaderComponent openNotification (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openNotification(undefined as any); }).not.toThrow();
  });
});
