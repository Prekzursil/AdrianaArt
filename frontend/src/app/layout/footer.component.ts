...
@Component({
  ...
  template: `
    <footer class="border-t border-border bg-background dark:border-slate-800 dark:bg-slate-900">
      <div
        class="max-w-6xl mx-auto px-4 sm:px-6 py-10 grid gap-10 lg:grid-cols-4 text-sm text-text dark:text-slate-300"
      >
        <section class="grid gap-4" aria-labelledby="footer-brand-heading">
          <div class="grid gap-1">
            <h2
              id="footer-brand-heading"
              class="text-lg font-semibold text-text-heading dark:text-slate-100"
            >
              {{ 'app.name' | translate }}
            </h2>
            <p class="text-text dark:text-slate-300">{{ 'app.tagline' | translate }}</p>
          </div>
          ...
          <div class="min-h-[2.5rem]">
            <ng-container *ngIf="socialLoading; else socialLinksReady">
              <div class="flex flex-wrap items-center gap-3" data-footer-social-loading="true">
                <span
                  class="h-5 w-24 rounded-full bg-surface dark:bg-slate-800 animate-pulse"
                ></span>
                <span
                  class="h-5 w-24 rounded-full bg-surface dark:bg-slate-800 animate-pulse"
                ></span>
                <span
                  class="h-5 w-16 rounded-full bg-surface dark:bg-slate-800 animate-pulse"
                ></span>
              </div>
            </ng-container>
            <ng-template #socialLinksReady>
              <nav class="flex flex-wrap items-center gap-4" aria-label="Footer social links">
                <div class="relative" data-footer-dropdown>
                  <button
                    type="button"
                    class="inline-flex items-center gap-1 font-medium hover:text-text-heading dark:hover:text-white"
                    (click)="toggleMenu('instagram')"
                    [attr.aria-expanded]="openMenu === 'instagram'"
                    aria-haspopup="menu"
                  >
                    {{ 'footer.instagram' | translate }}
                    <span class="text-xs text-text-muted dark:text-slate-400">▴</span>
                  </button>
                  <div
                    *ngIf="openMenu === 'instagram'"
                    class="absolute bottom-full left-0 mb-2 w-72 rounded-2xl border border-border bg-background shadow-xl overflow-hidden dark:border-slate-700 dark:bg-slate-900"
                    role="menu"
                  >
                    <a
                      *ngFor="let page of instagramPages"
                      class="flex items-center gap-3 px-3 py-2 text-sm text-text hover:bg-background hover:text-text-heading dark:text-slate-200 dark:hover:bg-slate-800 dark:hover:text-white"
                      [href]="page.url"
                      target="_blank"
                    ...
                    >
                      <ng-container *ngIf="page.thumbnailUrl; else instagramAvatar">
                        <img
                          ...
                          [alt]="page.label"
                          class="h-8 w-8 rounded-full border border-border object-cover dark:border-slate-700"
                      ...
                      <ng-template #instagramAvatar>
                        <span
                          class="h-8 w-8 rounded-full grid place-items-center text-xs font-semibold text-text-inverse"
                ...
                <a
                  class="font-medium hover:text-text-heading dark:hover:text-white"
                  routerLink="/contact"
        ...
        >
          <h2
            ...
            class="font-semibold text-text-heading dark:text-slate-100"
          >
            {{ 'footer.handcraftedArt' | translate }}
          </h2>
          <ng-container *ngIf="navLoading; else handcraftedLinksReady">
            <span
              class="h-4 w-28 rounded bg-surface dark:bg-slate-800 animate-pulse"
              data-footer-nav-loading="handcrafted"
            ></span>
            <span class="h-4 w-24 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-24 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-28 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
          </ng-container>
          <ng-template #handcraftedLinksReady>
            <nav class="grid gap-2" aria-labelledby="footer-handcrafted-heading">
              ...
              <ng-template #defaultHandcraftedLinks>
                <a class="hover:text-text-heading dark:hover:text-white" routerLink="/shop">{{
                  'nav.shop' | translate
                }}</a>
                <a class="hover:text-text-heading dark:hover:text-white" routerLink="/about">{{
                  'nav.about' | translate
                }}</a>
                <a class="hover:text-text-heading dark:hover:text-white" routerLink="/contact">{{
                  'nav.contact' | translate
                }}</a>
                <a class="hover:text-text-heading dark:hover:text-white" routerLink="/pages/terms">{{
                  'nav.terms' | translate
                }}</a>
              </ng-template>
            </nav>
        ...
        >
          <h2 id="footer-legal-heading" class="font-semibold text-text-heading dark:text-slate-100">
            {{ 'footer.legal' | translate }}
          </h2>
          <ng-container *ngIf="navLoading; else legalLinksReady">
            <span
              class="h-4 w-24 rounded bg-surface dark:bg-slate-800 animate-pulse"
              data-footer-nav-loading="legal"
            ></span>
            <span class="h-4 w-36 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-20 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
          </ng-container>
          <ng-template #legalLinksReady>
            <nav class="grid gap-2" aria-labelledby="footer-legal-heading">
              ...
              <ng-template #defaultLegalLinks>
                ...
                >
                <a class="hover:text-text-heading dark:hover:text-white" routerLink="/pages/anpc">{{
        ...
        >
          <h2 id="footer-company-heading" class="font-semibold text-text-heading dark:text-slate-100">
            {{ 'footer.companyInfo' | translate }}
          </h2>
          <ng-container *ngIf="companyLoading; else companyInfoReady">
            <span
              class="h-4 w-48 rounded bg-surface dark:bg-slate-800 animate-pulse"
              data-footer-company-loading="true"
            ></span>
            <span class="h-4 w-36 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-28 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-56 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-32 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
            <span class="h-4 w-40 rounded bg-surface dark:bg-slate-800 animate-pulse"></span>
          </ng-container>
          <ng-template #companyInfoReady>
            <p *ngIf="companyInfo.name" class="font-medium text-text dark:text-slate-100">
              {{ companyInfo.name }}
            </p>
        ...
        <section
          class="lg:col-span-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-border pt-6 dark:border-slate-800"
          aria-labelledby="footer-payments-heading"
        >
          <div class="flex flex-col sm:flex-row items-center gap-4">
            <h2
              id="footer-payments-heading"
              class="text-xs uppercase tracking-[0.2em] text-text dark:text-slate-300"
            >
              {{ 'footer.paymentsAccepted' | translate }}
            </h2>
            <div
              class="rounded-xl bg-background px-3 py-2 shadow-sm ring-1 ring-border dark:bg-slate-50 dark:ring-slate-200"
            >
              <img
