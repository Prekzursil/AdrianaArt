...
/** Golden WU admin-get-product-relationships -- getProductRelationships. */
describe('AdminService getProductRelationships (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getProductRelationships(''); }).not.toThrow();
  });
});
