...
/** Golden WU catalog-list-featured-collections -- listFeaturedCollections. */
describe('CatalogService listFeaturedCollections (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(CatalogService.prototype) as CatalogService;
    ...
    expect(() => { (cmp as any).listFeaturedCollections(undefined as any); }).not.toThrow();
  });
});
