...
/** Golden WU fx-rates-snapshot -- snapshot. */
describe('FxRatesService snapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(FxRatesService.prototype) as FxRatesService;
    ...
    expect(() => { (cmp as any).snapshot(); }).not.toThrow();
  });
});
