...
/** Golden WU admin-users-get-email-verification-history -- getEmailVerificationHistory. */
describe('AdminUsersService getEmailVerificationHistory (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminUsersService.prototype) as AdminUsersService;
    ...
    expect(() => { (cmp as any).getEmailVerificationHistory(''); }).not.toThrow();
  });
});
