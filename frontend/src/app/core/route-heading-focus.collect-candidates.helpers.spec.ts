...
/** Golden WU route-heading-focus-collect-candidates -- collectCandidates. */
describe('RouteHeadingFocusService collectCandidates (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).collectCandidates(); }).not.toThrow();
  });
});
