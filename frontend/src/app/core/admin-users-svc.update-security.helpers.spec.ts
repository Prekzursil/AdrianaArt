...
/** Golden WU admin-users-svc-update-security -- updateSecurity. */
describe('AdminUsersService updateSecurity (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).updateSecurity('', undefined as any); }).not.toThrow();
  });
});
