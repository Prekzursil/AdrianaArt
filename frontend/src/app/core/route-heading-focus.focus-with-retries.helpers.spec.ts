...
/** Golden WU route-heading-focus-focus-with-retries -- focusWithRetries. */
describe('RouteHeadingFocusService focusWithRetries (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).focusWithRetries(0, 0); }).not.toThrow();
  });
});
