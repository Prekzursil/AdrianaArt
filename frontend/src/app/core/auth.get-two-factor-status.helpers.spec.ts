...
/** Golden WU auth-get-two-factor-status -- getTwoFactorStatus. */
describe('AuthService getTwoFactorStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getTwoFactorStatus(); }).not.toThrow();
  });
});
