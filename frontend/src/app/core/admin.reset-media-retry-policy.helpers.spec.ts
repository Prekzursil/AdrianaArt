...
/** Golden WU admin-reset-media-retry-policy -- resetMediaRetryPolicy. */
describe('AdminService resetMediaRetryPolicy (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resetMediaRetryPolicy(undefined as any); }).not.toThrow();
  });
});
