...
/** Golden WU admin-orders-resend-order-confirmation-email -- resendOrderConfirmationEmail. */
describe('AdminOrdersService resendOrderConfirmationEmail (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminOrdersService.prototype) as AdminOrdersService;
    ...
    expect(() => { (cmp as any).resendOrderConfirmationEmail('', undefined as any); }).not.toThrow();
  });
});
