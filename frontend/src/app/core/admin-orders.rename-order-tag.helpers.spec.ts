...
/** Golden WU admin-orders-rename-order-tag -- renameOrderTag. */
describe('AdminOrdersService renameOrderTag (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).renameOrderTag(''); }).not.toThrow();
  });
});
