...
/** Golden WU cart-payment-intent -- paymentIntent. */
describe('CartApi paymentIntent (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(CartApi.prototype) as CartApi;
    ...
    expect(() => { (cmp as any).paymentIntent(); }).not.toThrow();
  });
});
