...
/** Golden WU markdown-render-with-sanitization-report -- renderWithSanitizationReport. */
describe('MarkdownService renderWithSanitizationReport (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(MarkdownService.prototype) as MarkdownService;
    ...
    expect(() => { (cmp as any).renderWithSanitizationReport(''); }).not.toThrow();
  });
});
