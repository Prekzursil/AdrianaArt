...
/** Golden WU admin-orders-download-shipping-label -- downloadShippingLabel. */
describe('AdminOrdersService downloadShippingLabel (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadShippingLabel('', undefined as any); }).not.toThrow();
  });
});
