...
/** Golden WU route-robots-resolve-policy -- resolvePolicy. */
describe('RouteRobotsService resolvePolicy (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(RouteRobotsService.prototype) as RouteRobotsService;
    ...
    expect(() => { (cmp as any).resolvePolicy(undefined as any); }).not.toThrow();
  });
});
