...
/** Golden WU auth-complete-google-link -- completeGoogleLink. */
describe('AuthService completeGoogleLink (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).completeGoogleLink('', '', ''); }).not.toThrow();
  });
});
