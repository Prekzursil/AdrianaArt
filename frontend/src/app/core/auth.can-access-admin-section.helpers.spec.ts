...
/** Golden WU auth-can-access-admin-section -- canAccessAdminSection. */
describe('AuthService canAccessAdminSection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).canAccessAdminSection(''); }).not.toThrow();
  });
});
