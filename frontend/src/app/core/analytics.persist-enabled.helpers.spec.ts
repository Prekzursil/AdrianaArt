...
/** Golden WU analytics-persist-enabled -- persistEnabled. */
describe('AnalyticsService persistEnabled (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistEnabled(false); }).not.toThrow();
  });
});
