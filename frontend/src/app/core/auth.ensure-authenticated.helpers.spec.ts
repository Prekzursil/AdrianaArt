...
/** Golden WU auth-ensure-authenticated -- ensureAuthenticated. */
describe('AuthService ensureAuthenticated (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).ensureAuthenticated(false); }).not.toThrow();
  });
});
