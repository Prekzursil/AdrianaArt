...
/** Golden WU catalog-request-back-in-stock -- requestBackInStock. */
describe('CatalogService requestBackInStock (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestBackInStock(''); }).not.toThrow();
  });
});
