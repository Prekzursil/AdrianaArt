...
/** Golden WU auth-revoke-other-sessions -- revokeOtherSessions. */
describe('AuthService revokeOtherSessions (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).revokeOtherSessions(''); }).not.toThrow();
  });
});
