...
/** Golden WU auth-request-password-reset -- requestPasswordReset. */
describe('AuthService requestPasswordReset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestPasswordReset(''); }).not.toThrow();
  });
});
