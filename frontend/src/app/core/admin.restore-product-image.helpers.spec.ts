...
/** Golden WU admin-restore-product-image -- restoreProductImage. */
describe('AdminService restoreProductImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restoreProductImage('', ''); }).not.toThrow();
  });
});
