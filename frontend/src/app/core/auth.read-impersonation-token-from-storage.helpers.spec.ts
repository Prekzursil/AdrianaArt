...
/** Golden WU auth-read-impersonation-token-from-storage -- readImpersonationTokenFromStorage. */
describe('AuthService readImpersonationTokenFromStorage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).readImpersonationTokenFromStorage(); }).not.toThrow();
  });
});
