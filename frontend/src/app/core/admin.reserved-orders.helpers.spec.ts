...
/** Golden WU admin-reserved-orders -- reservedOrders. */
describe('AdminService reservedOrders (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).reservedOrders(undefined as any); }).not.toThrow();
  });
});
