...
/** Golden WU analytics-persist-token -- persistToken. */
describe('AnalyticsService persistToken (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistToken('', undefined as any); }).not.toThrow();
  });
});
