...
/** Golden WU storefront-admin-mode-load-saved -- loadSaved. */
describe('StorefrontAdminModeService loadSaved (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(StorefrontAdminModeService.prototype) as StorefrontAdminModeService;
    ...
    expect(() => { (cmp as any).loadSaved(); }).not.toThrow();
  });
});
