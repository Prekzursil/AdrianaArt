...
/** Golden WU auth-read-from-storage -- readFromStorage. */
describe('AuthService readFromStorage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).readFromStorage('', undefined as any); }).not.toThrow();
  });
});
