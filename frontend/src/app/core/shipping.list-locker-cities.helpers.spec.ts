...
/** Golden WU shipping-list-locker-cities -- listLockerCities. */
describe('ShippingService listLockerCities (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(ShippingService.prototype) as ShippingService;
    ...
    expect(() => { (cmp as any).listLockerCities(''); }).not.toThrow();
  });
});
