...
/** Golden WU admin-request-media-usage-reconcile -- requestMediaUsageReconcile. */
describe('AdminService requestMediaUsageReconcile (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestMediaUsageReconcile(); }).not.toThrow();
  });
});
