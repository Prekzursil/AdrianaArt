...
/** Golden WU admin-send-scheduled-report -- sendScheduledReport. */
describe('AdminService sendScheduledReport (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).sendScheduledReport(false); }).not.toThrow();
  });
});
