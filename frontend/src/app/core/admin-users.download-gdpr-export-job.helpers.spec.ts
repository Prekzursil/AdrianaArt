...
/** Golden WU admin-users-download-gdpr-export-job -- downloadGdprExportJob. */
describe('AdminUsersService downloadGdprExportJob (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadGdprExportJob(''); }).not.toThrow();
  });
});
