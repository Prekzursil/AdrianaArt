...
/** Golden WU catalog-get-back-in-stock-status -- getBackInStockStatus. */
describe('CatalogService getBackInStockStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getBackInStockStatus(''); }).not.toThrow();
  });
});
