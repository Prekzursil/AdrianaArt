...
/** Golden WU blog-unhide-comment-admin -- unhideCommentAdmin. */
describe('BlogService unhideCommentAdmin (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).unhideCommentAdmin(''); }).not.toThrow();
  });
});
