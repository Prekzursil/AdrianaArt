...
/** Golden WU admin-list-media-job-events -- listMediaJobEvents. */
describe('AdminService listMediaJobEvents (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listMediaJobEvents('', 0); }).not.toThrow();
  });
});
