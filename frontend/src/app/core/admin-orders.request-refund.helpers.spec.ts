...
/** Golden WU admin-orders-request-refund -- requestRefund. */
describe('AdminOrdersService requestRefund (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestRefund('', undefined as any); }).not.toThrow();
  });
});
