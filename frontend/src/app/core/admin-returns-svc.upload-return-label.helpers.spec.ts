...
/** Golden WU admin-returns-svc-upload-return-label -- uploadReturnLabel. */
describe('AdminReturnsService uploadReturnLabel (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminReturnsService.prototype) as AdminReturnsService;
    ...
    expect(() => { (cmp as any).uploadReturnLabel('', undefined as any, false); }).not.toThrow();
  });
});
