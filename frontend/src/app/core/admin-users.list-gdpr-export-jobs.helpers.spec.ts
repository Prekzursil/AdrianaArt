...
/** Golden WU admin-users-list-gdpr-export-jobs -- listGdprExportJobs. */
describe('AdminUsersService listGdprExportJobs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listGdprExportJobs(''); }).not.toThrow();
  });
});
