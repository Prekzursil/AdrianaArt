...
/** Golden WU admin-users-resend-password-reset -- resendPasswordReset. */
describe('AdminUsersService resendPasswordReset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resendPasswordReset('', undefined as any); }).not.toThrow();
  });
});
