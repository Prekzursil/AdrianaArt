...
/** Golden WU admin-upsert-product-translation -- upsertProductTranslation. */
describe('AdminService upsertProductTranslation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertProductTranslation('', undefined as any, undefined as any); }).not.toThrow();
  });
});
