...
/** Golden WU admin-get-product-image-translations -- getProductImageTranslations. */
describe('AdminService getProductImageTranslations (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getProductImageTranslations('', ''); }).not.toThrow();
  });
});
