...
/** Golden WU admin-get-product-translations -- getProductTranslations. */
describe('AdminService getProductTranslations (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getProductTranslations(''); }).not.toThrow();
  });
});
