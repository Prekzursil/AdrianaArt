...
/** Golden WU admin-retry-media-jobs-bulk -- retryMediaJobsBulk. */
describe('AdminService retryMediaJobsBulk (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryMediaJobsBulk(''); }).not.toThrow();
  });
});
