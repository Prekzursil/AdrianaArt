...
/** Golden WU account-request-account-deletion -- requestAccountDeletion. */
describe('AccountService requestAccountDeletion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestAccountDeletion('', ''); }).not.toThrow();
  });
});
