...
/** Golden WU admin-client-error-logger-on-unhandled-rejection -- onUnhandledRejection. */
describe('AdminClientErrorLoggerService onUnhandledRejection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onUnhandledRejection({} as any); }).not.toThrow();
  });
});
