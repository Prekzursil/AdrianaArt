...
/** Golden WU admin-users-execute-gdpr-deletion -- executeGdprDeletion. */
describe('AdminUsersService executeGdprDeletion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).executeGdprDeletion('', ''); }).not.toThrow();
  });
});
