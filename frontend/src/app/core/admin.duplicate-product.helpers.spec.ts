...
/** Golden WU admin-duplicate-product -- duplicateProduct. */
describe('AdminService duplicateProduct (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).duplicateProduct('', undefined as any); }).not.toThrow();
  });
});
