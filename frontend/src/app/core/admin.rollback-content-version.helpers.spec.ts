...
/** Golden WU admin-rollback-content-version -- rollbackContentVersion. */
describe('AdminService rollbackContentVersion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).rollbackContentVersion('', 0); }).not.toThrow();
  });
});
