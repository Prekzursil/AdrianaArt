...
/** Golden WU account-request-order-cancellation -- requestOrderCancellation. */
describe('AccountService requestOrderCancellation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AccountService.prototype) as AccountService;
    ...
    expect(() => { (cmp as any).requestOrderCancellation('', ''); }).not.toThrow();
  });
});
