...
/** Golden WU admin-upload-product-image -- uploadProductImage. */
describe('AdminService uploadProductImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).uploadProductImage('', undefined as any); }).not.toThrow();
  });
});
