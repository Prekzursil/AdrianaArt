...
/** Golden WU admin-products-duplicate-check -- duplicateCheck. */
describe('AdminProductsService duplicateCheck (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminProductsService.prototype) as AdminProductsService;
    ...
    expect(() => { (cmp as any).duplicateCheck(''); }).not.toThrow();
  });
});
