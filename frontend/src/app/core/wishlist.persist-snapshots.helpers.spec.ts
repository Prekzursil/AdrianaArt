...
/** Golden WU wishlist-persist-snapshots -- persistSnapshots. */
describe('WishlistService persistSnapshots (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistSnapshots(''); }).not.toThrow();
  });
});
