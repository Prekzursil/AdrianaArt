...
/** Golden WU auth-delete-secondary-email -- deleteSecondaryEmail. */
describe('AuthService deleteSecondaryEmail (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).deleteSecondaryEmail('', ''); }).not.toThrow();
  });
});
