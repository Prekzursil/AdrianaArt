...
/** Golden WU wishlist-upsert-baseline -- upsertBaseline. */
describe('WishlistService upsertBaseline (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertBaseline(undefined as any); }).not.toThrow();
  });
});
