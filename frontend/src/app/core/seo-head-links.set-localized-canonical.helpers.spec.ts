...
/** Golden WU seo-head-links-set-localized-canonical -- setLocalizedCanonical. */
describe('SeoHeadLinksService setLocalizedCanonical (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(SeoHeadLinksService.prototype) as SeoHeadLinksService;
    ...
    expect(() => { (cmp as any).setLocalizedCanonical('', undefined as any, ''); }).not.toThrow();
  });
});
