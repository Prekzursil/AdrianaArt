...
/** Golden WU admin-users-list-high-aov-segment -- listHighAovSegment. */
describe('AdminUsersService listHighAovSegment (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listHighAovSegment(''); }).not.toThrow();
  });
});
