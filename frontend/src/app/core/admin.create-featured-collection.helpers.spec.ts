...
/** Golden WU admin-create-featured-collection -- createFeaturedCollection. */
describe('AdminService createFeaturedCollection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).createFeaturedCollection(undefined as any); }).not.toThrow();
  });
});
