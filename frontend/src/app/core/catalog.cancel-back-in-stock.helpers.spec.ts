...
/** Golden WU catalog-cancel-back-in-stock -- cancelBackInStock. */
describe('CatalogService cancelBackInStock (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).cancelBackInStock(''); }).not.toThrow();
  });
});
