...
/** Golden WU admin-preview-merge-category -- previewMergeCategory. */
describe('AdminService previewMergeCategory (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).previewMergeCategory('', ''); }).not.toThrow();
  });
});
