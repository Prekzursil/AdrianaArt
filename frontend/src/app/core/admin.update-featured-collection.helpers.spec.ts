...
/** Golden WU admin-update-featured-collection -- updateFeaturedCollection. */
describe('AdminService updateFeaturedCollection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).updateFeaturedCollection('', ''); }).not.toThrow();
  });
});
