...
/** Golden WU taxes-admin-update-group -- updateGroup. */
describe('TaxesAdminService updateGroup (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(TaxesAdminService.prototype) as TaxesAdminService;
    ...
    expect(() => { (cmp as any).updateGroup('', undefined as any); }).not.toThrow();
  });
});
