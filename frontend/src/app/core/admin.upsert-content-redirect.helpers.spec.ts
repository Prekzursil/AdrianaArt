...
/** Golden WU admin-upsert-content-redirect -- upsertContentRedirect. */
describe('AdminService upsertContentRedirect (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertContentRedirect(''); }).not.toThrow();
  });
});
