...
/** Golden WU admin-list-media-assets -- listMediaAssets. */
describe('AdminService listMediaAssets (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listMediaAssets(undefined as any); }).not.toThrow();
  });
});
