...
/** Golden WU ops-list-sameday-sync-runs -- listSamedaySyncRuns. */
describe('OpsService listSamedaySyncRuns (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listSamedaySyncRuns(0); }).not.toThrow();
  });
});
