...
/** Golden WU account-cancel-account-deletion -- cancelAccountDeletion. */
describe('AccountService cancelAccountDeletion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).cancelAccountDeletion(); }).not.toThrow();
  });
});
