...
/** Golden WU admin-users-resend-email-verification -- resendEmailVerification. */
describe('AdminUsersService resendEmailVerification (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resendEmailVerification(''); }).not.toThrow();
  });
});
