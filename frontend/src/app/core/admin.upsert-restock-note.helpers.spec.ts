...
/** Golden WU admin-upsert-restock-note -- upsertRestockNote. */
describe('AdminService upsertRestockNote (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertRestockNote(undefined as any); }).not.toThrow();
  });
});
