...
/** Golden WU account-get-deletion-status -- getDeletionStatus. */
describe('AccountService getDeletionStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getDeletionStatus(); }).not.toThrow();
  });
});
