...
/** Golden WU fx-admin-restore-override-from-audit -- restoreOverrideFromAudit. */
describe('FxAdminService restoreOverrideFromAudit (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(FxAdminService.prototype) as FxAdminService;
    ...
    expect(() => { (cmp as any).restoreOverrideFromAudit(''); }).not.toThrow();
  });
});
