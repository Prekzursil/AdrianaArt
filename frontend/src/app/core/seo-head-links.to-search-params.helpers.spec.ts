...
/** Golden WU seo-head-links-to-search-params -- toSearchParams. */
describe('SeoHeadLinksService toSearchParams (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toSearchParams(''); }).not.toThrow();
  });
});
