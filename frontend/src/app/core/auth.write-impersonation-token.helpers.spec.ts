...
/** Golden WU auth-write-impersonation-token -- writeImpersonationToken. */
describe('AuthService writeImpersonationToken (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).writeImpersonationToken(''); }).not.toThrow();
  });
});
