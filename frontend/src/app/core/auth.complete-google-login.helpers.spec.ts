...
/** Golden WU auth-complete-google-login -- completeGoogleLogin. */
describe('AuthService completeGoogleLogin (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).completeGoogleLogin('', ''); }).not.toThrow();
  });
});
