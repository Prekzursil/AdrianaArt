...
/** Golden WU admin-request-media-variant -- requestMediaVariant. */
describe('AdminService requestMediaVariant (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestMediaVariant('', ''); }).not.toThrow();
  });
});
