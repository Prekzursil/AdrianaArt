...
/** Golden WU admin-create-page-preview-token -- createPagePreviewToken. */
describe('AdminService createPagePreviewToken (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).createPagePreviewToken('', ''); }).not.toThrow();
  });
});
