...
/** Golden WU notifications-refresh-unread-count -- refreshUnreadCount. */
describe('NotificationsService refreshUnreadCount (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(NotificationsService.prototype) as NotificationsService;
    ...
    expect(() => { (cmp as any).refreshUnreadCount(); }).not.toThrow();
  });
});
