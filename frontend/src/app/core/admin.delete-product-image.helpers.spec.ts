...
/** Golden WU admin-delete-product-image -- deleteProductImage. */
describe('AdminService deleteProductImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).deleteProductImage('', ''); }).not.toThrow();
  });
});
