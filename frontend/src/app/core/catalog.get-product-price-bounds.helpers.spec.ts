...
/** Golden WU catalog-get-product-price-bounds -- getProductPriceBounds. */
describe('CatalogService getProductPriceBounds (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getProductPriceBounds(undefined as any); }).not.toThrow();
  });
});
