...
/** Golden WU auth-install-revalidation-hooks -- installRevalidationHooks. */
describe('AuthService installRevalidationHooks (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).installRevalidationHooks(); }).not.toThrow();
  });
});
