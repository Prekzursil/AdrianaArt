...
/** Golden WU admin-list-media-retry-policies -- listMediaRetryPolicies. */
describe('AdminService listMediaRetryPolicies (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listMediaRetryPolicies(); }).not.toThrow();
  });
});
