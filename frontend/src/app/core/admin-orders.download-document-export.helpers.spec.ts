...
/** Golden WU admin-orders-download-document-export -- downloadDocumentExport. */
describe('AdminOrdersService downloadDocumentExport (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadDocumentExport(''); }).not.toThrow();
  });
});
