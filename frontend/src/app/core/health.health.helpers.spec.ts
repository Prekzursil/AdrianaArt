...
/** Golden WU health-health -- health. */
describe('HealthService health (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(HealthService.prototype) as HealthService;
    ...
    expect(() => { (cmp as any).health(); }).not.toThrow();
  });
});
