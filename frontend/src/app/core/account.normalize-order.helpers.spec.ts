...
/** Golden WU account-normalize-order -- normalizeOrder. */
describe('AccountService normalizeOrder (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).normalizeOrder(undefined as any); }).not.toThrow();
  });
});
