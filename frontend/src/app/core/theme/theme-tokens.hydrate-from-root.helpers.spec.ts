...
/** Golden WU theme-tokens-hydrate-from-root -- hydrateFromRoot. */
describe('ThemeTokensService hydrateFromRoot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(ThemeTokensService.prototype) as ThemeTokensService;
    ...
    expect(() => { (cmp as any).hydrateFromRoot(); }).not.toThrow();
  });
});
