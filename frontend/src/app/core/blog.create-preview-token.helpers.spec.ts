...
/** Golden WU blog-create-preview-token -- createPreviewToken. */
describe('BlogService createPreviewToken (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).createPreviewToken('', ''); }).not.toThrow();
  });
});
