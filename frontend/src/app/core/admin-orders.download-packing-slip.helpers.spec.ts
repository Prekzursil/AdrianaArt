...
/** Golden WU admin-orders-download-packing-slip -- downloadPackingSlip. */
describe('AdminOrdersService downloadPackingSlip (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadPackingSlip(''); }).not.toThrow();
  });
});
