...
/** Golden WU auth-list-security-events -- listSecurityEvents. */
describe('AuthService listSecurityEvents (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listSecurityEvents(0); }).not.toThrow();
  });
});
