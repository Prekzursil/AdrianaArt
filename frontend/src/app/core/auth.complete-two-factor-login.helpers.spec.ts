...
/** Golden WU auth-complete-two-factor-login -- completeTwoFactorLogin. */
describe('AuthService completeTwoFactorLogin (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).completeTwoFactorLogin('', '', false); }).not.toThrow();
  });
});
