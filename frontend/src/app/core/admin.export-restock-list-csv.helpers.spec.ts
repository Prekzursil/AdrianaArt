...
/** Golden WU admin-export-restock-list-csv -- exportRestockListCsv. */
describe('AdminService exportRestockListCsv (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).exportRestockListCsv(0); }).not.toThrow();
  });
});
