...
/** Golden WU wishlist-normalize-product -- normalizeProduct. */
describe('WishlistService normalizeProduct (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(WishlistService.prototype) as WishlistService;
    ...
    expect(() => { (cmp as any).normalizeProduct(undefined as any); }).not.toThrow();
  });
});
