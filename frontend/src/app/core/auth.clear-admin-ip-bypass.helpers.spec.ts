...
/** Golden WU auth-clear-admin-ip-bypass -- clearAdminIpBypass. */
describe('AuthService clearAdminIpBypass (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).clearAdminIpBypass(); }).not.toThrow();
  });
});
