...
/** Golden WU auth-read-impersonation-token-from-url -- readImpersonationTokenFromUrl. */
describe('AuthService readImpersonationTokenFromUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).readImpersonationTokenFromUrl(); }).not.toThrow();
  });
});
