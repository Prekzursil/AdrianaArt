...
/** Golden WU support-submit-contact -- submitContact. */
describe('SupportService submitContact (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(SupportService.prototype) as SupportService;
    ...
    expect(() => { (cmp as any).submitContact(undefined as any); }).not.toThrow();
  });
});
