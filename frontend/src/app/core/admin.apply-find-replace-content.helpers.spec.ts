...
/** Golden WU admin-apply-find-replace-content -- applyFindReplaceContent. */
describe('AdminService applyFindReplaceContent (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyFindReplaceContent(undefined as any); }).not.toThrow();
  });
});
