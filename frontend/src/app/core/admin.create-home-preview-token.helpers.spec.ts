...
/** Golden WU admin-create-home-preview-token -- createHomePreviewToken. */
describe('AdminService createHomePreviewToken (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).createHomePreviewToken(''); }).not.toThrow();
  });
});
