...
/** Golden WU auth-clear-impersonation-token-from-url -- clearImpersonationTokenFromUrl. */
describe('AuthService clearImpersonationTokenFromUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).clearImpersonationTokenFromUrl(); }).not.toThrow();
  });
});
