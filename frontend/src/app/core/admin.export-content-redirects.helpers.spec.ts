...
/** Golden WU admin-export-content-redirects -- exportContentRedirects. */
describe('AdminService exportContentRedirects (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).exportContentRedirects(''); }).not.toThrow();
  });
});
