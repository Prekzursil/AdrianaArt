...
/** Golden WU admin-update-media-collection -- updateMediaCollection. */
describe('AdminService updateMediaCollection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).updateMediaCollection('', ''); }).not.toThrow();
  });
});
