...
/** Golden WU cart-schedule-sync-backend -- scheduleSyncBackend. */
describe('CartStore scheduleSyncBackend (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(CartStore.prototype) as CartStore;
    ...
    expect(() => { (cmp as any).scheduleSyncBackend(undefined as any); }).not.toThrow();
  });
});
