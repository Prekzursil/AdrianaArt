...
/** Golden WU admin-recent-merge -- merge. */
describe('AdminRecentService merge (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminRecentService.prototype) as AdminRecentService;
    ...
    expect(() => { (cmp as any).merge(undefined as any, undefined as any); }).not.toThrow();
  });
});
