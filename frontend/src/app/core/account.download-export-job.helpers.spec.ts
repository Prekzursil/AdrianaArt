...
/** Golden WU account-download-export-job -- downloadExportJob. */
describe('AccountService downloadExportJob (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadExportJob(''); }).not.toThrow();
  });
});
