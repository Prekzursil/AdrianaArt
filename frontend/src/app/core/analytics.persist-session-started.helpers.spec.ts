...
/** Golden WU analytics-persist-session-started -- persistSessionStarted. */
describe('AnalyticsService persistSessionStarted (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AnalyticsService.prototype) as AnalyticsService;
    ...
    expect(() => { (cmp as any).persistSessionStarted(false); }).not.toThrow();
  });
});
