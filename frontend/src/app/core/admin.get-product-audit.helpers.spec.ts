...
/** Golden WU admin-get-product-audit -- getProductAudit. */
describe('AdminService getProductAudit (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getProductAudit('', undefined as any); }).not.toThrow();
  });
});
