...
/** Golden WU admin-list-media-collections -- listMediaCollections. */
describe('AdminService listMediaCollections (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listMediaCollections(); }).not.toThrow();
  });
});
