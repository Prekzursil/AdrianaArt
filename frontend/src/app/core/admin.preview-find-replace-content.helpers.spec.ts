...
/** Golden WU admin-preview-find-replace-content -- previewFindReplaceContent. */
describe('AdminService previewFindReplaceContent (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).previewFindReplaceContent(undefined as any); }).not.toThrow();
  });
});
