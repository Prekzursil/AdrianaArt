...
/** Golden WU admin-orders-list-email-events -- listEmailEvents. */
describe('AdminOrdersService listEmailEvents (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listEmailEvents('', 0); }).not.toThrow();
  });
});
