...
/** Golden WU auth-has-valid-refresh-token -- hasValidRefreshToken. */
describe('AuthService hasValidRefreshToken (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).hasValidRefreshToken(); }).not.toThrow();
  });
});
