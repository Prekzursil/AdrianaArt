...
/** Golden WU admin-fetch-social-thumbnail -- fetchSocialThumbnail. */
describe('AdminService fetchSocialThumbnail (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).fetchSocialThumbnail(''); }).not.toThrow();
  });
});
