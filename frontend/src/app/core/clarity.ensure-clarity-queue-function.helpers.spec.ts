...
/** Golden WU clarity-ensure-clarity-queue-function -- ensureClarityQueueFunction. */
describe('ClarityService ensureClarityQueueFunction (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(ClarityService.prototype) as ClarityService;
    ...
    expect(() => { (cmp as any).ensureClarityQueueFunction(); }).not.toThrow();
  });
});
