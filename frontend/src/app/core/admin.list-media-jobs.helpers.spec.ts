...
/** Golden WU admin-list-media-jobs -- listMediaJobs. */
describe('AdminService listMediaJobs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listMediaJobs(undefined as any); }).not.toThrow();
  });
});
