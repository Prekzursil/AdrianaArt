...
/** Golden WU admin-replace-media-collection-items -- replaceMediaCollectionItems. */
describe('AdminService replaceMediaCollectionItems (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).replaceMediaCollectionItems('', ''); }).not.toThrow();
  });
});
