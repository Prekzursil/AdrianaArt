...
/** Golden WU admin-invalidate-coupon-stripe-mappings -- invalidateCouponStripeMappings. */
describe('AdminService invalidateCouponStripeMappings (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).invalidateCouponStripeMappings(''); }).not.toThrow();
  });
});
