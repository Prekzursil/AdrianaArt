...
/** Golden WU admin-list-content-versions -- listContentVersions. */
describe('AdminService listContentVersions (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listContentVersions(''); }).not.toThrow();
  });
});
