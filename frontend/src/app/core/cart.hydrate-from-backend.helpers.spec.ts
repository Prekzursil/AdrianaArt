...
/** Golden WU cart-hydrate-from-backend -- hydrateFromBackend. */
describe('CartStore hydrateFromBackend (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).hydrateFromBackend(undefined as any); }).not.toThrow();
  });
});
