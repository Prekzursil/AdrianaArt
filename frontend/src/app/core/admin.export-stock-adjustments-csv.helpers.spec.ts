...
/** Golden WU admin-export-stock-adjustments-csv -- exportStockAdjustmentsCsv. */
describe('AdminService exportStockAdjustmentsCsv (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).exportStockAdjustmentsCsv(''); }).not.toThrow();
  });
});
