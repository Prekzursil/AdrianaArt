...
/** Golden WU auth-request-secondary-email-verification -- requestSecondaryEmailVerification. */
describe('AuthService requestSecondaryEmailVerification (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestSecondaryEmailVerification('', ''); }).not.toThrow();
  });
});
