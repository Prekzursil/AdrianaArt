...
/** Golden WU theme-service-get-saved-preference -- getSavedPreference. */
describe('ThemeService getSavedPreference (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(ThemeService.prototype) as ThemeService;
    ...
    expect(() => { cmp.getSavedPreference(); }).not.toThrow();
  });
});
