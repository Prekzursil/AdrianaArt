...
/** Golden WU admin-support-update-canned-response -- updateCannedResponse. */
describe('AdminSupportService updateCannedResponse (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminSupportService.prototype) as AdminSupportService;
    ...
    expect(() => { (cmp as any).updateCannedResponse('', undefined as any); }).not.toThrow();
  });
});
