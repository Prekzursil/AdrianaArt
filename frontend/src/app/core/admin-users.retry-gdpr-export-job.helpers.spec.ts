...
/** Golden WU admin-users-retry-gdpr-export-job -- retryGdprExportJob. */
describe('AdminUsersService retryGdprExportJob (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryGdprExportJob(''); }).not.toThrow();
  });
});
