...
/** Golden WU admin-users-cancel-gdpr-deletion -- cancelGdprDeletion. */
describe('AdminUsersService cancelGdprDeletion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).cancelGdprDeletion(''); }).not.toThrow();
  });
});
