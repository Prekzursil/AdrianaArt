...
/** Golden WU recently-viewed-normalize -- normalize. */
describe('RecentlyViewedService normalize (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(RecentlyViewedService.prototype) as RecentlyViewedService;
    ...
    expect(() => { (cmp as any).normalize(undefined as any); }).not.toThrow();
  });
});
