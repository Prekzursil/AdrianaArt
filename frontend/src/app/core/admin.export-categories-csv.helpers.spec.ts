...
/** Golden WU admin-export-categories-csv -- exportCategoriesCsv. */
describe('AdminService exportCategoriesCsv (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).exportCategoriesCsv(undefined as any); }).not.toThrow();
  });
});
