...
/** Golden WU admin-get-product-image-stats -- getProductImageStats. */
describe('AdminService getProductImageStats (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getProductImageStats('', ''); }).not.toThrow();
  });
});
