...
/** Golden WU missing-translation-handle -- handle. */
describe('AppMissingTranslationHandler handle (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AppMissingTranslationHandler.prototype) as AppMissingTranslationHandler;
    ...
    expect(() => { (cmp as any).handle(undefined as any); }).not.toThrow();
  });
});
