...
/** Golden WU server-translate-get-translation -- getTranslation. */
describe('ServerTranslateLoader getTranslation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(ServerTranslateLoader.prototype) as ServerTranslateLoader;
    ...
    expect(() => { (cmp as any).getTranslation(''); }).not.toThrow();
  });
});
