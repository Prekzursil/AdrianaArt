...
/** Golden WU admin-users-override-email-verification -- overrideEmailVerification. */
describe('AdminUsersService overrideEmailVerification (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).overrideEmailVerification('', ''); }).not.toThrow();
  });
});
