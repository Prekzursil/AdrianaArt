...
/** Golden WU structured-data-set-route-schemas -- setRouteSchemas. */
describe('StructuredDataService setRouteSchemas (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(StructuredDataService.prototype) as StructuredDataService;
    ...
    expect(() => { (cmp as any).setRouteSchemas(''); }).not.toThrow();
  });
});
