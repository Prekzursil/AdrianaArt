...
/** Golden WU admin-list-deleted-product-images -- listDeletedProductImages. */
describe('AdminService listDeletedProductImages (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listDeletedProductImages(''); }).not.toThrow();
  });
});
