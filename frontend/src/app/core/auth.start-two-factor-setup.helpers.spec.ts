...
/** Golden WU auth-start-two-factor-setup -- startTwoFactorSetup. */
describe('AuthService startTwoFactorSetup (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).startTwoFactorSetup(''); }).not.toThrow();
  });
});
