...
/** Golden WU admin-soft-delete-media-asset -- softDeleteMediaAsset. */
describe('AdminService softDeleteMediaAsset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).softDeleteMediaAsset(''); }).not.toThrow();
  });
});
