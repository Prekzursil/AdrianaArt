...
/** Golden WU auth-update-training-mode -- updateTrainingMode. */
describe('AuthService updateTrainingMode (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).updateTrainingMode(false); }).not.toThrow();
  });
});
