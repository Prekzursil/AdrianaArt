...
/** Golden WU auth-confirm-password-reset -- confirmPasswordReset. */
describe('AuthService confirmPasswordReset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).confirmPasswordReset('', ''); }).not.toThrow();
  });
});
