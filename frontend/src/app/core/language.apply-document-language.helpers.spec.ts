...
/** Golden WU language-apply-document-language -- applyDocumentLanguage. */
describe('LanguageService applyDocumentLanguage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(LanguageService.prototype) as LanguageService;
    ...
    expect(() => { (cmp as any).applyDocumentLanguage(undefined as any); }).not.toThrow();
  });
});
