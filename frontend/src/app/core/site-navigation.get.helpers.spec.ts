...
/** Golden WU site-navigation-get -- get. */
describe('SiteNavigationService get (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(SiteNavigationService.prototype) as SiteNavigationService;
    ...
    expect(() => { (cmp as any).get(); }).not.toThrow();
  });
});
