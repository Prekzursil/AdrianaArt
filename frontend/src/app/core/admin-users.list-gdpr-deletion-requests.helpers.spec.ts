...
/** Golden WU admin-users-list-gdpr-deletion-requests -- listGdprDeletionRequests. */
describe('AdminUsersService listGdprDeletionRequests (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listGdprDeletionRequests(''); }).not.toThrow();
  });
});
