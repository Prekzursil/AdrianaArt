...
/** Golden WU blog-resolve-comment-flags-admin -- resolveCommentFlagsAdmin. */
describe('BlogService resolveCommentFlagsAdmin (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(BlogService.prototype) as BlogService;
    ...
    expect(() => { (cmp as any).resolveCommentFlagsAdmin(''); }).not.toThrow();
  });
});
