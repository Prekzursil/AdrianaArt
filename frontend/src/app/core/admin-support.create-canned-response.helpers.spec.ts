...
/** Golden WU admin-support-create-canned-response -- createCannedResponse. */
describe('AdminSupportService createCannedResponse (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).createCannedResponse(''); }).not.toThrow();
  });
});
