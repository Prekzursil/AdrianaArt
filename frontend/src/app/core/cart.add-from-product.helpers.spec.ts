...
/** Golden WU cart-add-from-product -- addFromProduct. */
describe('CartStore addFromProduct (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).addFromProduct(undefined as any); }).not.toThrow();
  });
});
