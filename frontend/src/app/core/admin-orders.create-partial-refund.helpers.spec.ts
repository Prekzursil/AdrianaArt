...
/** Golden WU admin-orders-create-partial-refund -- createPartialRefund. */
describe('AdminOrdersService createPartialRefund (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).createPartialRefund('', ''); }).not.toThrow();
  });
});
