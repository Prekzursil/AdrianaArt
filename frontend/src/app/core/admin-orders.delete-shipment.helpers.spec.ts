...
/** Golden WU admin-orders-delete-shipment -- deleteShipment. */
describe('AdminOrdersService deleteShipment (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).deleteShipment('', '', false); }).not.toThrow();
  });
});
