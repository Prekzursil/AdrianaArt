...
/** Golden WU admin-orders-send-delivery-email -- sendDeliveryEmail. */
describe('AdminOrdersService sendDeliveryEmail (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).sendDeliveryEmail(''); }).not.toThrow();
  });
});
