...
/** Golden WU auth-confirm-email-verification -- confirmEmailVerification. */
describe('AuthService confirmEmailVerification (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).confirmEmailVerification(''); }).not.toThrow();
  });
});
