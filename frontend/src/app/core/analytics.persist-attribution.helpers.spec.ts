...
/** Golden WU analytics-persist-attribution -- persistAttribution. */
describe('AnalyticsService persistAttribution (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistAttribution(''); }).not.toThrow();
  });
});
