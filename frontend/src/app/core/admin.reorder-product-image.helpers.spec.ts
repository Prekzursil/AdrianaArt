...
/** Golden WU admin-reorder-product-image -- reorderProductImage. */
describe('AdminService reorderProductImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).reorderProductImage('', '', 0, undefined as any); }).not.toThrow();
  });
});
