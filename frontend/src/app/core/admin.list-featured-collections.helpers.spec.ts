...
/** Golden WU admin-list-featured-collections -- listFeaturedCollections. */
describe('AdminService listFeaturedCollections (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).listFeaturedCollections(); }).not.toThrow();
  });
});
