...
/** Golden WU analytics-read-session-started -- readSessionStarted. */
describe('AnalyticsService readSessionStarted (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).readSessionStarted(); }).not.toThrow();
  });
});
