...
/** Golden WU admin-orders-download-batch-shipping-labels-zip -- downloadBatchShippingLabelsZip. */
describe('AdminOrdersService downloadBatchShippingLabelsZip (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadBatchShippingLabelsZip(''); }).not.toThrow();
  });
});
