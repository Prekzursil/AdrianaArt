...
/** Golden WU admin-orders-download-receipt-pdf -- downloadReceiptPdf. */
describe('AdminOrdersService downloadReceiptPdf (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).downloadReceiptPdf(''); }).not.toThrow();
  });
});
