...
/** Golden WU auth-request-email-verification -- requestEmailVerification. */
describe('AuthService requestEmailVerification (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestEmailVerification(''); }).not.toThrow();
  });
});
