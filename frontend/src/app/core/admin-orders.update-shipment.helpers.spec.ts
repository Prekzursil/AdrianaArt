...
/** Golden WU admin-orders-update-shipment -- updateShipment. */
describe('AdminOrdersService updateShipment (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).updateShipment('', '', undefined as any, false); }).not.toThrow();
  });
});
