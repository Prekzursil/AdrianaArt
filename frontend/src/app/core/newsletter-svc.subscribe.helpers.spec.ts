...
/** Golden WU newsletter-svc-subscribe -- subscribe. */
describe('NewsletterService subscribe (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(NewsletterService.prototype) as NewsletterService;
    ...
    expect(() => { (cmp as any).subscribe('', undefined as any); }).not.toThrow();
  });
});
