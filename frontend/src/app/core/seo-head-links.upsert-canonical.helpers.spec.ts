...
/** Golden WU seo-head-links-upsert-canonical -- upsertCanonical. */
describe('SeoHeadLinksService upsertCanonical (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertCanonical(''); }).not.toThrow();
  });
});
