...
/** Golden WU admin-upload-product-image-with-progress -- uploadProductImageWithProgress. */
describe('AdminService uploadProductImageWithProgress (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).uploadProductImageWithProgress('', undefined as any); }).not.toThrow();
  });
});
