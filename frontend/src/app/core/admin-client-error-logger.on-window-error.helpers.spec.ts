...
/** Golden WU admin-client-error-logger-on-window-error -- onWindowError. */
describe('AdminClientErrorLoggerService onWindowError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onWindowError({} as any); }).not.toThrow();
  });
});
