...
/** Golden WU admin-finalize-media-asset -- finalizeMediaAsset. */
describe('AdminService finalizeMediaAsset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).finalizeMediaAsset('', undefined as any); }).not.toThrow();
  });
});
