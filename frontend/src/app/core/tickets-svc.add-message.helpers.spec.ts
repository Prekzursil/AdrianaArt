...
/** Golden WU tickets-svc-add-message -- addMessage. */
describe('TicketsService addMessage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(TicketsService.prototype) as TicketsService;
    ...
    expect(() => { (cmp as any).addMessage('', ''); }).not.toThrow();
  });
});
