...
/** Golden WU auth-start-passkey-login -- startPasskeyLogin. */
describe('AuthService startPasskeyLogin (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).startPasskeyLogin(undefined as any, false); }).not.toThrow();
  });
});
