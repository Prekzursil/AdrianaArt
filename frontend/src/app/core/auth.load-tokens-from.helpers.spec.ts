...
/** Golden WU auth-load-tokens-from -- loadTokensFrom. */
describe('AuthService loadTokensFrom (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadTokensFrom(undefined as any); }).not.toThrow();
  });
});
