...
/** Golden WU admin-orders-remove-order-tag -- removeOrderTag. */
describe('AdminOrdersService removeOrderTag (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeOrderTag('', '', false); }).not.toThrow();
  });
});
