...
/** Golden WU auth-make-secondary-email-primary -- makeSecondaryEmailPrimary. */
describe('AuthService makeSecondaryEmailPrimary (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).makeSecondaryEmailPrimary('', ''); }).not.toThrow();
  });
});
