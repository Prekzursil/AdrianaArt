...
/** Golden WU seo-head-links-upsert-alternate -- upsertAlternate. */
describe('SeoHeadLinksService upsertAlternate (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertAlternate(undefined as any, ''); }).not.toThrow();
  });
});
