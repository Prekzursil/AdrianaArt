...
/** Golden WU product-card-undo-status-change -- undoStatusChange. */
describe('ProductCardComponent undoStatusChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).undoStatusChange('', '', '', undefined as any); }).not.toThrow();
  });
});
