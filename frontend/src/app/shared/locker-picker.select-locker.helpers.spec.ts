...
/** Golden WU locker-picker-select-locker -- selectLocker. */
describe('LockerPickerComponent selectLocker (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).selectLocker(undefined as any); }).not.toThrow();
  });
});
