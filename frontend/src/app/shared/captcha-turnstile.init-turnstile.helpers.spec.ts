...
/** Golden WU captcha-turnstile-init-turnstile -- initTurnstile. */
describe('CaptchaTurnstileComponent initTurnstile (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).initTurnstile(); }).not.toThrow();
  });
});
