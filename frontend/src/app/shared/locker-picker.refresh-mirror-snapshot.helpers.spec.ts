...
/** Golden WU locker-picker-refresh-mirror-snapshot -- refreshMirrorSnapshot. */
describe('LockerPickerComponent refreshMirrorSnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).refreshMirrorSnapshot(); }).not.toThrow();
  });
});
