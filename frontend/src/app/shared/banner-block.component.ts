...
@Component({
  ...
  template: `
    <div class="grid gap-6" [ngClass]="wrapperClass()">
      <ng-container *ngIf="slide.variant === 'split'; else fullTpl">
        <div class="grid gap-6 lg:gap-10 lg:grid-cols-[1fr_1.4fr] items-start">
          <div class="grid gap-4">
            <p
              *ngIf="tagline"
              class="font-cinzel font-semibold text-[28px] tracking-[0.3em] text-text-muted dark:text-slate-400"
            >
              {{ tagline }}
            </p>
            <h2
              *ngIf="slide.headline"
              class="text-3xl sm:text-4xl lg:text-5xl font-semibold leading-tight text-text-heading dark:text-slate-50"
            >
              {{ slide.headline }}
            </h2>
            <p *ngIf="slide.subheadline" class="text-lg text-text dark:text-slate-300">
              {{ slide.subheadline }}
            </p>
          ...
          <div class="relative">
            ...
            <ng-template #splitPlaceholder>
              <div
                [ngClass]="imageClass()"
                class="bg-gradient-to-br from-surface-inverse via-surface-inverse to-surface-inverse grid place-items-center text-text-inverse text-xl font-semibold"
              >
                Banner image slot
              </div>
            </ng-template>
      ...
      <ng-template #fullTpl>
        <div
          class="relative overflow-hidden rounded-3xl border border-border bg-background shadow-sm dark:border-slate-800 dark:bg-slate-900"
        >
...
export class BannerBlockComponent {
  ...
  overlayClass(): string {
    return this.slide.text_style === 'light'
      ? 'bg-gradient-to-t from-surface-inverse/70 via-surface-inverse/30 to-transparent'
      : 'bg-gradient-to-t from-background/80 via-background/20 to-transparent dark:from-slate-900/70 dark:via-slate-900/30';
  }

  textClass(): string {
    return this.slide.text_style === 'light' ? 'text-text-inverse' : 'text-text-heading dark:text-slate-50';
  }
