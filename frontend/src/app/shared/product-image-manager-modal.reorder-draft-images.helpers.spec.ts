...
/** Golden WU product-image-manager-modal-reorder-draft-images -- reorderDraftImages. */
describe('ProductImageManagerModalComponent reorderDraftImages (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).reorderDraftImages('', ''); }).not.toThrow();
  });
});
