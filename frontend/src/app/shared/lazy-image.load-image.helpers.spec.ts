...
/** Golden WU lazy-image-load-image -- loadImage. */
describe('LazyImageDirective loadImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(LazyImageDirective.prototype) as LazyImageDirective;
    ...
    expect(() => { (cmp as any).loadImage(); }).not.toThrow();
  });
});
