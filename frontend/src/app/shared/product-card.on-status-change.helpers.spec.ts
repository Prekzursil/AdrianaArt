...
/** Golden WU product-card-on-status-change -- onStatusChange. */
describe('ProductCardComponent onStatusChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onStatusChange({} as any); }).not.toThrow();
  });
});
