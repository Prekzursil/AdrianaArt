...
/** Golden WU modal-restore-previous-focus -- restorePreviousFocus. */
describe('ModalComponent restorePreviousFocus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restorePreviousFocus(); }).not.toThrow();
  });
});
