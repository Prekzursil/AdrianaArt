...
/** Golden WU product-image-manager-modal-on-drag-end -- onDragEnd. */
describe('ProductImageManagerModalComponent onDragEnd (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onDragEnd(); }).not.toThrow();
  });
});
