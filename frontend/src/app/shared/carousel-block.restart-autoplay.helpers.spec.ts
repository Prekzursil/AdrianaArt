...
/** Golden WU carousel-block-restart-autoplay -- restartAutoplay. */
describe('CarouselBlockComponent restartAutoplay (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restartAutoplay(); }).not.toThrow();
  });
});
