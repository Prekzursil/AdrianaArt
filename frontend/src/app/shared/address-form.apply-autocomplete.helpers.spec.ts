...
/** Golden WU address-form-apply-autocomplete -- applyAutocomplete. */
describe('AddressFormComponent applyAutocomplete (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyAutocomplete(''); }).not.toThrow();
  });
});
