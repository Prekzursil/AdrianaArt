...
/** Golden WU localized-currency-get-formatter -- getFormatter. */
describe('LocalizedCurrencyPipe getFormatter (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(LocalizedCurrencyPipe.prototype) as LocalizedCurrencyPipe;
    ...
    expect(() => { (cmp as any).getFormatter('', '', 0, 0); }).not.toThrow();
  });
});
