...
/** Golden WU product-image-manager-modal-load-meta -- loadMeta. */
describe('ProductImageManagerModalComponent loadMeta (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadMeta(''); }).not.toThrow();
  });
});
