...
/** Golden WU locker-picker-use-my-location -- useMyLocation. */
describe('LockerPickerComponent useMyLocation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).useMyLocation(); }).not.toThrow();
  });
});
