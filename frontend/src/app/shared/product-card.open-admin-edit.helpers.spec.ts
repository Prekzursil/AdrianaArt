...
/** Golden WU product-card-open-admin-edit -- openAdminEdit. */
describe('ProductCardComponent openAdminEdit (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openAdminEdit({} as any); }).not.toThrow();
  });
});
