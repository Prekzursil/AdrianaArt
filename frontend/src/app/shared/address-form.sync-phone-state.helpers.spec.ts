...
/** Golden WU address-form-sync-phone-state -- syncPhoneState. */
describe('AddressFormComponent syncPhoneState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).syncPhoneState(); }).not.toThrow();
  });
});
