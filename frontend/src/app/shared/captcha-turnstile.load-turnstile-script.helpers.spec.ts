...
/** Golden WU captcha-turnstile-load-turnstile-script -- loadTurnstileScript. */
describe('CaptchaTurnstileComponent loadTurnstileScript (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadTurnstileScript(); }).not.toThrow();
  });
});
