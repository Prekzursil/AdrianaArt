...
/** Golden WU localized-currency-transform -- transform. */
describe('LocalizedCurrencyPipe transform (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).transform(undefined as any, '', ''); }).not.toThrow();
  });
});
