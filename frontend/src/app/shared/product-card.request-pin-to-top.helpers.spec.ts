...
/** Golden WU product-card-request-pin-to-top -- requestPinToTop. */
describe('ProductCardComponent requestPinToTop (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).requestPinToTop(); }).not.toThrow();
  });
});
