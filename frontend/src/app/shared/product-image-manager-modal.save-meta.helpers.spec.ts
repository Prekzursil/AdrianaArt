...
/** Golden WU product-image-manager-modal-save-meta -- saveMeta. */
describe('ProductImageManagerModalComponent saveMeta (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).saveMeta(); }).not.toThrow();
  });
});
