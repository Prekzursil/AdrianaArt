...
/** Golden WU nav-drawer-on-language-change -- onLanguageChange. */
describe('NavDrawerComponent onLanguageChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onLanguageChange(''); }).not.toThrow();
  });
});
