...
/** Golden WU product-card-undo-inline-update -- undoInlineUpdate. */
describe('ProductCardComponent undoInlineUpdate (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).undoInlineUpdate(0, 0, 0, 0); }).not.toThrow();
  });
});
