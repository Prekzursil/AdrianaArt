    ...
    <article class="group grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm hover:-translate-y-1 hover:shadow-md transition">
      ...
      <div class="grid gap-1">
        <div class="flex items-center justify-between gap-2">
          <a [routerLink]="['/products', product.slug]" class="font-semibold text-slate-900 line-clamp-1">
            {{ product.name }}
          </a>
        ...
        <div class="flex items-center gap-2 text-xs text-amber-700" *ngIf="product.rating_count">
