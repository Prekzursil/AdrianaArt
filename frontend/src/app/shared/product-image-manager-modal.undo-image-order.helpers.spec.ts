...
/** Golden WU product-image-manager-modal-undo-image-order -- undoImageOrder. */
describe('ProductImageManagerModalComponent undoImageOrder (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).undoImageOrder('', ''); }).not.toThrow();
  });
});
