...
/** Golden WU modal-get-focusable-elements -- getFocusableElements. */
describe('ModalComponent getFocusableElements (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).getFocusableElements(undefined as any); }).not.toThrow();
  });
});
