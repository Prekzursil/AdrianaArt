...
/** Golden WU locker-picker-search-first-result -- searchFirstResult. */
describe('LockerPickerComponent searchFirstResult (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).searchFirstResult(); }).not.toThrow();
  });
});
