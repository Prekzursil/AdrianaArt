...
/** Golden WU legal-consent-modal-handle-accept -- handleAccept. */
describe('LegalConsentModalComponent handleAccept (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).handleAccept(); }).not.toThrow();
  });
});
