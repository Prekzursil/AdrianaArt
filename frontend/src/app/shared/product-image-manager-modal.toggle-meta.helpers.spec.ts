...
/** Golden WU product-image-manager-modal-toggle-meta -- toggleMeta. */
describe('ProductImageManagerModalComponent toggleMeta (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toggleMeta(''); }).not.toThrow();
  });
});
