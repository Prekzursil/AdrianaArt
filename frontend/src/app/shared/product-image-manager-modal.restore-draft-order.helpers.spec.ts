...
/** Golden WU product-image-manager-modal-restore-draft-order -- restoreDraftOrder. */
describe('ProductImageManagerModalComponent restoreDraftOrder (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restoreDraftOrder(''); }).not.toThrow();
  });
});
