...
/** Golden WU product-image-manager-modal-on-drag-start -- onDragStart. */
describe('ProductImageManagerModalComponent onDragStart (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onDragStart({} as any, ''); }).not.toThrow();
  });
});
