...
/** Golden WU product-image-manager-modal-apply-local-meta-to-draft-image -- applyLocalMetaToDraftImage. */
describe('ProductImageManagerModalComponent applyLocalMetaToDraftImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyLocalMetaToDraftImage(''); }).not.toThrow();
  });
});
