...
/** Golden WU product-image-manager-modal-on-drop -- onDrop. */
describe('ProductImageManagerModalComponent onDrop (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onDrop({} as any, ''); }).not.toThrow();
  });
});
