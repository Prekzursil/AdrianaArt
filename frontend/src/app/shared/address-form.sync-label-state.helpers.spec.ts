...
/** Golden WU address-form-sync-label-state -- syncLabelState. */
describe('AddressFormComponent syncLabelState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).syncLabelState(); }).not.toThrow();
  });
});
