...
/** Golden WU locker-picker-apply-location -- applyLocation. */
describe('LockerPickerComponent applyLocation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyLocation(undefined as any); }).not.toThrow();
  });
});
