...
/** Golden WU product-card-on-primary-click -- onPrimaryClick. */
describe('ProductCardComponent onPrimaryClick (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onPrimaryClick({} as any); }).not.toThrow();
  });
});
