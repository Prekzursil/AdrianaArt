...
/** Golden WU rich-editor-sync-theme-class -- syncThemeClass. */
describe('RichEditorComponent syncThemeClass (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(RichEditorComponent.prototype) as RichEditorComponent;
    ...
    expect(() => { (cmp as any).syncThemeClass(); }).not.toThrow();
  });
});
