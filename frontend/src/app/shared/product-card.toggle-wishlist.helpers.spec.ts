...
/** Golden WU product-card-toggle-wishlist -- toggleWishlist. */
describe('ProductCardComponent toggleWishlist (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toggleWishlist({} as any); }).not.toThrow();
  });
});
