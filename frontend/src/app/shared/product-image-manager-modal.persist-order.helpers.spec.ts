...
/** Golden WU product-image-manager-modal-persist-order -- persistOrder. */
describe('ProductImageManagerModalComponent persistOrder (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistOrder(''); }).not.toThrow();
  });
});
