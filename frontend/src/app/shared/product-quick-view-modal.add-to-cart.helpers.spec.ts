...
/** Golden WU product-quick-view-modal-add-to-cart -- addToCart. */
describe('ProductQuickViewModalComponent addToCart (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).addToCart(); }).not.toThrow();
  });
});
