...
/** Golden WU locker-picker-fetch-locations -- fetchLocations. */
describe('LockerPickerComponent fetchLocations (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).fetchLocations('', false); }).not.toThrow();
  });
});
