...
/** Golden WU carousel-block-start-autoplay -- startAutoplay. */
describe('CarouselBlockComponent startAutoplay (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).startAutoplay(); }).not.toThrow();
  });
});
