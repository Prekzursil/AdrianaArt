...
/** Golden WU product-image-manager-modal-clear-meta -- clearMeta. */
describe('ProductImageManagerModalComponent clearMeta (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).clearMeta(); }).not.toThrow();
  });
});
