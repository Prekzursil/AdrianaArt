...
/** Golden WU cms-announcement-bar-load -- load. */
describe('CmsAnnouncementBarComponent load (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(CmsAnnouncementBarComponent.prototype) as CmsAnnouncementBarComponent;
    ...
    expect(() => { (cmp as any).load(); }).not.toThrow();
  });
});
