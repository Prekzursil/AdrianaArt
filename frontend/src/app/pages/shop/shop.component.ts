  ...
  template: `
    <app-container classes="pt-10 pb-24 lg:pb-10 grid gap-6">
      ...
      <div class="grid gap-8 lg:grid-cols-[280px_1fr]">
        <aside
          id="shop-filters"
          class="border border-border rounded-2xl p-4 bg-background h-fit space-y-6 scroll-mt-24 dark:border-slate-800 dark:bg-slate-900"
        >
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              <h2 class="text-lg font-semibold text-text-heading dark:text-slate-50">
                {{ 'shop.filters' | translate }}
              </h2>
              <button
                class="text-sm text-accent font-medium dark:text-indigo-300"
                type="button"
                (click)="resetFilters()"
              >
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between gap-2">
              <p class="text-sm font-semibold text-text dark:text-slate-200">
                {{ 'shop.categories' | translate }}
              </p>
              <button
                *ngIf="canEditCategories()"
                type="button"
                class="rounded-full border border-border bg-background px-2 py-1 text-[11px] font-semibold text-text shadow-sm transition hover:bg-background disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
                [disabled]="reorderSaving() || renameSaving || renameLoading || createSaving"
                (click)="toggleCreateRootCategory()"
              >
                {{
                  isCreatingRootCategory()
                    ? ('adminUi.common.cancel' | translate)
                    : ('adminUi.storefront.categories.addRoot' | translate)
                }}
              </button>
            </div>
            <p *ngIf="canEditCategories()" class="text-xs text-text-muted dark:text-slate-400">
              {{ 'adminUi.storefront.categories.dragHint' | translate }}
            </p>
            <div class="space-y-2 max-h-48 overflow-auto pr-1">
              <label class="flex items-center gap-2 text-sm text-text dark:text-slate-300">
                <input
                  type="radio"
                  name="category"
                  class="h-4 w-4 rounded border-border dark:border-slate-600"
                  value=""
              ...
              <div
                *ngIf="isCreatingRootCategory()"
                class="grid gap-3 rounded-xl border border-border bg-background p-3 dark:border-slate-700 dark:bg-slate-900"
              >
                <app-input
                ...
                ></app-input>
                <p *ngIf="createError" class="text-xs text-rose-700 dark:text-rose-300">
                  {{ createError }}
                </p>
                ...
                *ngFor="let category of rootCategories"
                ...
                [ngClass]="
                  dragOverRootCategorySlug === category.slug
                    ? 'rounded-lg bg-background dark:bg-slate-800/60'
                    : ''
                "
                [attr.draggable]="
                  canEditCategories() &&
                  editingCategorySlug !== category.slug &&
                  !isCreatingAnyCategory()
                    ? 'true'
                    : null
                "
              ...
              >
                <span class="flex items-center gap-2 text-sm text-text dark:text-slate-300">
                  <span
                    *ngIf="canEditCategories()"
                    class="text-text-muted dark:text-slate-500 select-none cursor-grab"
                    aria-hidden="true"
                  >
                    ⋮⋮
                  </span>
                  ...
                  <span
                    *ngIf="canEditCategories() && category.is_visible === false"
                    class="rounded-full bg-surface px-2 py-0.5 text-[10px] font-semibold text-text dark:bg-slate-800 dark:text-slate-200"
                  >
                ...
                <div
                  *ngIf="editingCategorySlug === category.slug"
                  class="ml-6 grid gap-3 rounded-xl border border-border bg-background p-3 dark:border-slate-700 dark:bg-slate-900"
                  (click)="$event.stopPropagation()"
                >
                  <div *ngIf="renameLoading" class="text-xs text-text dark:text-slate-300">
                  ...
                  <div *ngIf="!renameLoading" class="grid gap-3">
                    ...
                    ></app-input>
                    <div
                      class="grid gap-2 rounded-xl border border-border bg-background p-3 dark:border-slate-700 dark:bg-slate-800/40"
                    >
                      <p class="text-xs font-semibold text-text dark:text-slate-200">
                        {{ 'adminUi.storefront.categories.images' | translate }}
                      </p>
                      <div class="grid gap-3">
                        <div class="grid gap-2">
                          <p class="text-xs font-semibold text-text dark:text-slate-300">
                          ...
                          <div class="flex items-center gap-3">
                            <img
                              *ngIf="category.thumbnail_url"
                              ...
                              class="h-12 w-12 rounded-lg border border-border object-cover dark:border-slate-700"
                              [alt]="category.name"
                            />
                            <input
                              type="file"
                              accept="image/*"
                              class="block w-full text-xs text-text file:mr-3 file:rounded-full file:border-0 file:bg-surface-inverse file:px-3 file:py-2 file:text-xs file:font-semibold file:text-text-inverse hover:file:bg-surface-inverse dark:text-slate-200 dark:file:bg-slate-50 dark:file:text-slate-900 dark:hover:file:bg-white"
                              [disabled]="
                                renameSaving ||
                                createSaving ||
                                categoryImageSavingSlug === category.slug
                              "
                        ...
                        <div class="grid gap-2">
                          ...
                          <p
                            *ngIf="category.banner_url"
                            class="text-xs text-text dark:text-slate-300"
                          >
                      ...
                      <p
                        *ngIf="categoryImageError"
                        class="text-xs text-rose-700 dark:text-rose-300"
                      >
                        {{ categoryImageError }}
                      </p>
                    </div>
                    <p *ngIf="renameError" class="text-xs text-rose-700 dark:text-rose-300">
                      {{ renameError }}
                    </p>
                    <div class="flex flex-wrap justify-end gap-2">
                      <app-button
                    ...
                    <div
                      class="grid gap-3 rounded-xl border border-border bg-background p-3 dark:border-slate-700 dark:bg-slate-900"
                    >
                      <p class="text-xs font-semibold text-text dark:text-slate-100">
                        {{ 'adminUi.storefront.categories.mergeTitle' | translate }}
                      </p>
                      <label class="grid gap-1">
                        <span class="text-xs font-semibold text-text dark:text-slate-300">
                          {{ 'adminUi.storefront.categories.mergeInto' | translate }}
                        </span>
                        <select
                          class="h-10 rounded-xl border border-border bg-background px-3 text-sm text-text-heading shadow-sm focus:outline-none focus:ring-2 focus:ring-accent/40 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                      ...
                      <p *ngIf="mergePreview" class="text-xs text-text dark:text-slate-300">
                        {{
                          'adminUi.storefront.categories.mergePreviewInfo'
                            | translate
                              : {
                                  products: mergePreview.product_count,
                                  children: mergePreview.child_count,
                                }
                        }}
                      </p>
                      ...
                      <p *ngIf="deletePreview" class="text-xs text-rose-800 dark:text-rose-200">
                        {{
                          'adminUi.storefront.categories.deletePreviewInfo'
                            | translate
                              : {
                                  products: deletePreview.product_count,
                                  children: deletePreview.child_count,
                                }
                        }}
                      </p>
                ...
                <div
                  *ngIf="
                    ...
                    (getSubcategories(category).length || canEditCategories())
                  "
                ...
                >
                  <div class="flex items-center justify-between gap-2">
                    ...
                    >
                      {{
                        isCreatingSubcategory(category.slug)
                          ? ('adminUi.common.cancel' | translate)
                          : ('adminUi.storefront.categories.addSub' | translate)
                      }}
                    </button>
                  </div>
                  <div
                    ...
                    <button
                      ...
                      [ngClass]="
                        !activeSubcategorySlug
                          ? 'bg-surface-inverse text-text-inverse border-border dark:bg-slate-50 dark:text-slate-900 dark:border-slate-50'
                          : 'border-border text-text hover:border-border hover:text-text-heading dark:border-slate-700 dark:text-slate-200 dark:hover:border-slate-500 dark:hover:text-slate-50'
                      "
          ...
          <div class="space-y-3">
            ...
            <div class="grid gap-3">
              <p id="shop-price-status" class="sr-only">
                {{
                  'shop.priceRangeStatus'
                    | translate: { min: filters.min_price, max: filters.max_price }
                }}
              </p>
              ...
              <p id="shop-price-hint" class="text-xs text-text-muted dark:text-slate-400">
        ...
        <section class="grid gap-6">
          <div
            id="shop-actions"
            class="flex flex-col sm:flex-row sm:items-center gap-3 justify-between scroll-mt-24"
          >
            <div class="flex items-center gap-3">
              <input
                class="w-64 max-w-full rounded-lg border border-border bg-background px-3 py-2 text-text-heading shadow-sm focus:border-border focus:outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-400"
                [placeholder]="'shop.searchPlaceholder' | translate"
            ...
            <div class="flex flex-wrap items-center justify-end gap-3">
              ...
              <button
                *ngIf="canEditProducts()"
                type="button"
                class="rounded-full border border-border bg-background px-3 py-2 text-xs font-semibold text-text shadow-sm transition hover:bg-background disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
                [disabled]="bulkSaving()"
                (click)="toggleBulkSelectMode()"
              >
                {{
                  bulkSelectMode()
                    ? ('adminUi.storefront.products.bulkDone' | translate)
                    : ('adminUi.storefront.products.bulkSelect' | translate)
                }}
              </button>
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
            <ng-container *ngIf="filterChips() as chips">
              <div *ngIf="chips.length" class="flex flex-wrap items-center gap-2">
                <p
                  class="text-xs font-semibold uppercase tracking-[0.2em] text-text-muted dark:text-slate-400"
                >
                ...
                <button
                  *ngFor="let chip of chips; trackBy: trackChip"
                  type="button"
                  class="inline-flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1 text-xs font-semibold text-text hover:border-border hover:bg-background dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-slate-500 dark:hover:bg-slate-800"
                  (click)="removeChip(chip)"
                  [attr.aria-label]="'shop.removeFilter' | translate: { filter: chip.label }"
                >
                  <span>{{ chip.label }}</span>
                  <span aria-hidden="true" class="text-text-muted dark:text-slate-400">×</span>
                </button>
                <button
                  type="button"
                  class="ml-1 text-xs font-semibold text-accent hover:text-accent dark:text-indigo-300 dark:hover:text-indigo-200"
                  (click)="resetFilters()"
                >
            ...
            <div
              *ngIf="!loading() && !hasError() && resultsMetaParams() as meta"
              class="text-sm text-text dark:text-slate-300"
            >
          ...
          <div
            *ngIf="canEditProducts() && bulkSelectMode()"
            class="grid gap-4 rounded-2xl border border-border bg-background p-4 dark:border-slate-800 dark:bg-slate-900"
          >
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div
                class="flex flex-wrap items-center gap-2 text-sm text-text dark:text-slate-200"
              >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <label class="grid gap-1 text-xs font-semibold text-text dark:text-slate-200">
                ...
                <select
                  class="h-10 rounded-lg border border-border bg-background px-3 text-sm font-normal text-text-heading shadow-sm focus:border-border focus:outline-none disabled:opacity-60 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                  [disabled]="bulkSaving()"
                  [(ngModel)]="bulkStatus"
                >
                  <option value="">
                    {{ 'adminUi.storefront.products.bulkStatusPlaceholder' | translate }}
                  </option>
              ...
              <label class="grid gap-1 text-xs font-semibold text-text dark:text-slate-200">
                ...
                >
                  <option value="">
                    {{ 'adminUi.storefront.products.bulkCategoryPlaceholder' | translate }}
                  </option>
              ...
              <label class="grid gap-1 text-xs font-semibold text-text dark:text-slate-200">
                ...
                >
                  <option value="">
                    {{ 'adminUi.storefront.products.bulkFeaturedPlaceholder' | translate }}
                  </option>
                  <option value="true">
                    {{ 'adminUi.storefront.products.bulkFeaturedOn' | translate }}
                  </option>
                  <option value="false">
                    {{ 'adminUi.storefront.products.bulkFeaturedOff' | translate }}
                  </option>
                </select>
              </label>
            </div>

            <p *ngIf="bulkEditError" class="text-xs text-rose-700 dark:text-rose-300">
              {{ bulkEditError }}
            </p>
          </div>

          <p *ngIf="canReorderProducts()" class="text-xs text-text-muted dark:text-slate-400">
          ...
          <div *ngIf="!loading() && !hasError() && products.length === 0" class="grid gap-4">
            ...
            <div *ngIf="rootCategories.length" class="mt-1 grid gap-2">
              ...
              <div class="flex flex-wrap gap-2">
                <button
                  type="button"
                  class="rounded-full border border-border bg-background px-3 py-1 text-xs font-semibold text-text hover:border-border hover:bg-background dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:border-slate-500 dark:hover:bg-slate-800"
          ...
          >
            ...
            >
              <input
                *ngIf="bulkSelectMode()"
                type="checkbox"
                class="absolute left-3 top-3 z-20 h-5 w-5 rounded border-border bg-background/90 shadow-sm accent-accent dark:border-slate-600 dark:bg-slate-900/80"
          ...
          <div
            *ngIf="pageMeta"
            class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm text-text dark:text-slate-300"
          >
            <div class="flex flex-wrap items-center gap-3">
              <div
                class="inline-flex items-center overflow-hidden rounded-full border border-border bg-background dark:border-slate-700 dark:bg-slate-900"
              >
                <button
                  type="button"
                  ...
                  [ngClass]="
                    paginationMode === 'pages'
                      ? 'bg-surface-inverse text-text-inverse dark:bg-slate-50 dark:text-slate-900'
                      : 'text-text hover:bg-background dark:text-slate-200 dark:hover:bg-slate-800'
                  "
              ...
              <div *ngIf="paginationMode === 'load_more'">
                {{
                  'shop.loadedCount'
                    | translate: { shown: products.length, total: pageMeta.total_items }
                }}
              </div>
            </div>
            ...
            <div *ngIf="paginationMode === 'load_more'" class="flex">
              <button
                type="button"
                class="rounded-full border border-border bg-background px-4 py-2 text-sm font-semibold text-text-heading hover:bg-background disabled:opacity-60 disabled:cursor-not-allowed dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50 dark:hover:bg-slate-800"
    ...
    <div
      class="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-background/90 backdrop-blur dark:border-slate-800 dark:bg-slate-900/80 lg:hidden"
    >
      <div class="mx-auto max-w-6xl px-4 sm:px-6 py-3 grid grid-cols-2 gap-3">
        <button
          type="button"
          class="h-11 w-full rounded-full bg-surface-inverse text-text-inverse text-sm font-semibold hover:bg-surface-inverse dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-white"
        ...
        <button
          type="button"
          class="h-11 w-full rounded-full border border-border bg-background text-text-heading text-sm font-semibold hover:bg-background dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50 dark:hover:bg-slate-800"
          (click)="scrollToSort()"
        >
          {{ 'shop.sort' | translate }}
        </button>
...
export class ShopComponent implements OnInit, OnDestroy {
  ...
  crumbs = [{ label: 'nav.home', url: '/' }, { label: 'nav.shop' }];
  ...
  canReorderProducts(): boolean {
    ...
    const loadedAll =
      totalPages === 1 ||
      (this.paginationMode === 'load_more' &&
        page >= totalPages &&
        this.products.length >= totalItems);
  ...
  toggleCategoryVisibility(event: MouseEvent, category: Category): void {
    ...
    this.admin
      .updateCategory(slug, { is_visible: !currentlyVisible }, { source: 'storefront' })
      .subscribe({
        next: () => {
          this.visibilitySavingSlug = null;
          this.toast.success(
            this.translate.instant('adminUi.storefront.categories.visibilitySuccess'),
          );
          this.fetchCategories();
        },
        error: () => {
  ...
  saveRenameCategory(): void {
    ...
    this.admin
      ...
      .pipe(
        switchMap(() =>
          forkJoin([
            this.admin.upsertCategoryTranslation(
              slug,
              'ro',
              { name: nameRo },
              { source: 'storefront' },
  ...
  onCategoryImageSelected(event: Event, slug: string, kind: 'thumbnail' | 'banner'): void {
    ...
    this.admin.uploadCategoryImage(desired, kind, file, { source: 'storefront' }).subscribe({
      next: () => {
        this.categoryImageSavingSlug = null;
        this.toast.success(
          this.translate.instant('adminUi.storefront.categories.imageUploadSuccess'),
        );
        this.fetchCategories();
      },
      error: () => {
        this.categoryImageSavingSlug = null;
        this.categoryImageError = this.translate.instant(
          'adminUi.storefront.categories.imageUploadError',
        );
      },
    });
  }
  ...
  mergeCategory(category: Category): void {
    ...
    if (!this.mergePreview || !this.mergePreview.can_merge) {
      this.mergeError = this.translate.instant(
        'adminUi.storefront.categories.mergePreviewRequired',
      );
      return;
    }
    ...
    this.admin.previewDeleteCategory(slug).subscribe({
      next: (preview) => {
        ...
        if (!preview.can_delete) {
          this.deleteError = this.translate.instant(
            'adminUi.storefront.categories.deleteNotAllowed',
            {
      ...
      error: () => {
        this.deletePreviewLoading = false;
        this.deleteError = this.translate.instant(
          'adminUi.storefront.categories.deletePreviewError',
        );
      },
    });
  }
  ...
  deleteCategorySafe(category: Category): void {
    ...
    if (!this.deletePreview || !this.deletePreview.can_delete) {
      this.deleteError = this.translate.instant(
        'adminUi.storefront.categories.deletePreviewRequired',
      );
      return;
    }
    const slug = (category?.slug || '').trim();
  ...
  saveCreateCategory(): void {
    ...
    this.admin
      .createCategory(
        { name: nameRo, sort_order: sortOrder, parent_id: parentId },
        { source: 'storefront' },
      )
      .pipe(
        switchMap((created) =>
  ...
  private undoRootCategoryOrder(previousSlugs: string[], currentSlugs: string[]): void {
    if (this.reorderSaving()) return;
    const payload = previousSlugs
      .map((slug, idx) => ({ slug, sort_order: idx }))
  ...
  private fetchProducts(append = false): void {
    ...
    const categorySlug = isSale
      ? undefined
      : this.activeSubcategorySlug || this.activeCategorySlug || undefined;
    ...
    this.catalog
      ...
      .subscribe({
        next: (response) => {
          ...
          this.products =
            append && this.products.length ? [...this.products, ...incoming] : incoming;
          if (!append) {
            this.clearBulkSelection();
            this.bulkEditError = '';
          }
          ...
          if (isSale) {
            ...
            this.crumbs = [
              ...
              { label: 'nav.shop', url: '/shop' },
              {
                label: cat?.name ?? this.activeCategorySlug,
                url: `/shop/${this.activeCategorySlug}`,
              },
              ...(sub ? [{ label: sub.name ?? sub.slug }] : []),
            ];
          } else {
            this.crumbs = [{ label: 'nav.home', url: '/' }, { label: 'nav.shop' }];
          }
        ...
        error: () => {
          ...
          if (append) {
            ...
            this.toast.error(
              this.translate.instant('shop.errorTitle'),
              this.translate.instant('shop.errorCopy'),
            );
            return;
          }
          this.products = [];
  ...
  setMetaTags(): void {
    ...
    const path = this.activeCategorySlug
      ? `/shop/${encodeURIComponent(this.activeCategorySlug)}`
      : '/shop';
  ...
  private rebuildCategoryTree(): void {
    ...
    this.rootCategories = this.categories.filter((c) => !c.parent_id).sort(sortByOrderThenName);
  }
  ...
  private buildQueryParams(): Params {
    return {
      ...
      sub:
        this.activeCategorySlug && this.activeCategorySlug !== 'sale'
  ...
  private syncFiltersFromQuery(params: Params): void {
    ...
    this.filters.sort = allowedSorts.includes(rawSort as SortOption)
  ...
  filterChips(): ShopFilterChip[] {
    ...
    if (this.activeCategorySlug) {
      if (this.activeCategorySlug === 'sale') {
        chips.push({
          id: 'category:sale',
          type: 'category',
          label: this.translate.instant('shop.sale'),
        });
      } else {
        const category = this.categoriesBySlug.get(this.activeCategorySlug);
        chips.push({
    ...
    if (hasMin || hasMax) {
      chips.push({
        ...
        type: 'price',
        label: this.translate.instant('shop.priceChip', {
          min: this.filters.min_price,
  ...
  resultsMetaParams(): { total: number; from: number; to: number } | null {
    ...
    if (!Number.isFinite(total) || !Number.isFinite(page) || !Number.isFinite(limit) || limit <= 0)
      return null;
