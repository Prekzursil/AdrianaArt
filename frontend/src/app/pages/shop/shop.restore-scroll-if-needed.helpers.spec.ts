...
/** Golden WU shop-restore-scroll-if-needed -- restoreScrollIfNeeded. */
describe('ShopComponent restoreScrollIfNeeded (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => (cmp as any).restoreScrollIfNeeded()).not.toThrow();
  });
});
