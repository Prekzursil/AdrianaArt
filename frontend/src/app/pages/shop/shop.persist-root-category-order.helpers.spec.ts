...
/** Golden WU shop-persist-root-category-order -- persistRootCategoryOrder. */
describe('ShopComponent persistRootCategoryOrder (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.persistRootCategoryOrder([] as any); }).not.toThrow();
  });
});
