...
/** Golden WU shop-cmp-reorder-root-categories -- reorderRootCategories. */
describe('ShopComponent reorderRootCategories (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).reorderRootCategories('', ''); }).not.toThrow();
  });
});
