...
/** Golden WU cart-cmp-add-saved-for-later -- addSavedForLater. */
describe('CartComponent addSavedForLater (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).addSavedForLater(undefined as any); }).not.toThrow();
  });
});
