...
/** Golden WU checkout-cmp-submit-checkout-request -- submitCheckoutRequest. */
describe('CheckoutComponent submitCheckoutRequest (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).submitCheckoutRequest('', ''); }).not.toThrow();
  });
});
