...
/** Golden WU paypal-mock-complete -- complete. */
describe('PayPalMockComponent complete (golden WU)', () => {
  it('returns early on guard', () => {
    const cmp = Object.create(PayPalMockComponent.prototype) as PayPalMockComponent;
    ...
    expect(() => (cmp as any).complete('success' as any)).not.toThrow();
  });
});
