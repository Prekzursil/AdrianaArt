...
/** Golden WU checkout-cmp-apply-saved-address-to-shipping -- applySavedAddressToShipping. */
describe('CheckoutComponent applySavedAddressToShipping (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applySavedAddressToShipping(undefined as any); }).not.toThrow();
  });
});
