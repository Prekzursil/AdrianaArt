...
/** Golden WU checkout-cmp-load-saved-addresses -- loadSavedAddresses. */
describe('CheckoutComponent loadSavedAddresses (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadSavedAddresses(false); }).not.toThrow();
  });
});
