...
/** Golden WU checkout-shipping-step-guest-last-name -- guestLastName. */
describe('CheckoutShippingStepComponent guestLastName (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).guestLastName(); }).not.toThrow();
  });
});
