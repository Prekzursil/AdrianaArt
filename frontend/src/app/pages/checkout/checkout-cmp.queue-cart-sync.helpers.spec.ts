...
/** Golden WU checkout-cmp-queue-cart-sync -- queueCartSync. */
describe('CheckoutComponent queueCartSync (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).queueCartSync(undefined as any, false); }).not.toThrow();
  });
});
