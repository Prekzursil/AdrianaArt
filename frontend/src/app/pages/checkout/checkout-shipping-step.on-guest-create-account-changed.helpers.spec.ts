...
/** Golden WU checkout-shipping-step-on-guest-create-account-changed -- onGuestCreateAccountChanged. */
describe('CheckoutShippingStepComponent onGuestCreateAccountChanged (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onGuestCreateAccountChanged(false); }).not.toThrow();
  });
});
