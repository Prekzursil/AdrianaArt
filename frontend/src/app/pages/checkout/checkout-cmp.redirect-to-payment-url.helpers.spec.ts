...
/** Golden WU checkout-cmp-redirect-to-payment-url -- redirectToPaymentUrl. */
describe('CheckoutComponent redirectToPaymentUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).redirectToPaymentUrl(undefined as any, ''); }).not.toThrow();
  });
});
