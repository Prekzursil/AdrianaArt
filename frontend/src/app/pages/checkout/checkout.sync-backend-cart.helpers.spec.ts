...
/** Golden WU checkout-sync-backend-cart -- syncBackendCart. */
describe('CheckoutComponent syncBackendCart (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).syncBackendCart(undefined as any); }).not.toThrow();
  });
});
