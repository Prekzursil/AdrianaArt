...
/** Golden WU checkout-redirect-to-cart-if-empty -- redirectToCartIfEmpty. */
describe('CheckoutComponent redirectToCartIfEmpty (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => (cmp as any).redirectToCartIfEmpty()).not.toThrow();
  });
});
