...
/** Golden WU login-normalize-next-url -- normalizeNextUrl. */
describe('LoginComponent normalizeNextUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).normalizeNextUrl(undefined as any); }).not.toThrow();
  });
});
