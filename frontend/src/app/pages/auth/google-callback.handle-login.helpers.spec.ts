...
/** Golden WU google-callback-handle-login -- handleLogin. */
describe('GoogleCallbackComponent handleLogin (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(GoogleCallbackComponent.prototype) as GoogleCallbackComponent;
    ...
    expect(() => { (cmp as any).handleLogin('', ''); }).not.toThrow();
  });
});
