...
/** Golden WU verify-email-navigate-silently -- navigateSilently. */
describe('VerifyEmailComponent navigateSilently (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).navigateSilently(''); }).not.toThrow();
  });
});
