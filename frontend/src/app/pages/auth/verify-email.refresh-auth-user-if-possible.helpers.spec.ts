...
/** Golden WU verify-email-refresh-auth-user-if-possible -- refreshAuthUserIfPossible. */
describe('VerifyEmailComponent refreshAuthUserIfPossible (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(VerifyEmailComponent.prototype) as VerifyEmailComponent;
    ...
    expect(() => { (cmp as any).refreshAuthUserIfPossible(); }).not.toThrow();
  });
});
