...
/** Golden WU product-load-back-in-stock-status -- loadBackInStockStatus. */
describe('ProductComponent loadBackInStockStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadBackInStockStatus(); }).not.toThrow();
  });
});
