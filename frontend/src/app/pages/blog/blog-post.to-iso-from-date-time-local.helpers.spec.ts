...
/** Golden WU blog-post-to-iso-from-date-time-local -- toIsoFromDateTimeLocal. */
describe('BlogPostComponent toIsoFromDateTimeLocal (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toIsoFromDateTimeLocal(''); }).not.toThrow();
  });
});
