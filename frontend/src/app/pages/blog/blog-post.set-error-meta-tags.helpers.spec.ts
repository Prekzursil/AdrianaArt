...
/** Golden WU blog-post-set-error-meta-tags -- setErrorMetaTags. */
describe('BlogPostComponent setErrorMetaTags (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setErrorMetaTags(); }).not.toThrow();
  });
});
