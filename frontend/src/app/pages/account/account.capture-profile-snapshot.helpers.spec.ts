...
/** Golden WU account-capture-profile-snapshot -- captureProfileSnapshot. */
describe('AccountState captureProfileSnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).captureProfileSnapshot(); }).not.toThrow();
  });
});
