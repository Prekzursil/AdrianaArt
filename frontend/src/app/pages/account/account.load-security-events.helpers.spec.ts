...
/** Golden WU account-load-security-events -- loadSecurityEvents. */
describe('AccountState loadSecurityEvents (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadSecurityEvents(false); }).not.toThrow();
  });
});
