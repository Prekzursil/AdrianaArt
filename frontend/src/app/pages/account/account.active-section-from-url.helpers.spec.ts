...
/** Golden WU account-active-section-from-url -- activeSectionFromUrl. */
describe('AccountState activeSectionFromUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).activeSectionFromUrl(''); }).not.toThrow();
  });
});
