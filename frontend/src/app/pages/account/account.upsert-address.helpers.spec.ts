...
/** Golden WU account-upsert-address -- upsertAddress. */
describe('AccountState upsertAddress (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).upsertAddress(undefined as any); }).not.toThrow();
  });
});
