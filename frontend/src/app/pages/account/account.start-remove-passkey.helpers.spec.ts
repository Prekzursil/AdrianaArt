...
/** Golden WU account-start-remove-passkey -- startRemovePasskey. */
describe('AccountState startRemovePasskey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).startRemovePasskey(''); }).not.toThrow();
  });
});
