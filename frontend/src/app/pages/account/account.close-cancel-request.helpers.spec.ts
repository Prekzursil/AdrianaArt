...
/** Golden WU account-close-cancel-request -- closeCancelRequest. */
describe('AccountState closeCancelRequest (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).closeCancelRequest(); }).not.toThrow();
  });
});
