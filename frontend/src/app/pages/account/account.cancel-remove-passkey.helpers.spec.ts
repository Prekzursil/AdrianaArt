...
/** Golden WU account-cancel-remove-passkey -- cancelRemovePasskey. */
describe('AccountState cancelRemovePasskey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).cancelRemovePasskey(); }).not.toThrow();
  });
});
