...
/** Golden WU account-remember-last-visited-section -- rememberLastVisitedSection. */
describe('AccountState rememberLastVisitedSection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).rememberLastVisitedSection(undefined as any); }).not.toThrow();
  });
});
