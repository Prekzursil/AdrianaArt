...
/** Golden WU account-load-coupons-count -- loadCouponsCount. */
describe('AccountState loadCouponsCount (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadCouponsCount(false); }).not.toThrow();
  });
});
