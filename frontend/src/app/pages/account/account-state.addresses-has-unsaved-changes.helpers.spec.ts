...
/** Golden WU account-state-addresses-has-unsaved-changes -- addressesHasUnsavedChanges. */
describe('AccountState addressesHasUnsavedChanges (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.addressesHasUnsavedChanges(); }).not.toThrow();
  });
});
