...
/** Golden WU account-load-deletion-status -- loadDeletionStatus. */
describe('AccountState loadDeletionStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadDeletionStatus(); }).not.toThrow();
  });
});
