...
/** Golden WU account-capture-notification-snapshot -- captureNotificationSnapshot. */
describe('AccountState captureNotificationSnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).captureNotificationSnapshot(); }).not.toThrow();
  });
});
