...
/** Golden WU account-same-profile-snapshot -- sameProfileSnapshot. */
describe('AccountState sameProfileSnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).sameProfileSnapshot(undefined as any, undefined as any); }).not.toThrow();
  });
});
