...
/** Golden WU account-count-available-coupons -- countAvailableCoupons. */
describe('AccountState countAvailableCoupons (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).countAvailableCoupons(undefined as any); }).not.toThrow();
  });
});
