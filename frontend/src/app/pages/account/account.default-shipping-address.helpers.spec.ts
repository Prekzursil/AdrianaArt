...
/** Golden WU account-default-shipping-address -- defaultShippingAddress. */
describe('AccountState defaultShippingAddress (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).defaultShippingAddress(); }).not.toThrow();
  });
});
