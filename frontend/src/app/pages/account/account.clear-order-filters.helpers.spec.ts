...
/** Golden WU account-clear-order-filters -- clearOrderFilters. */
describe('AccountState clearOrderFilters (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).clearOrderFilters(); }).not.toThrow();
  });
});
