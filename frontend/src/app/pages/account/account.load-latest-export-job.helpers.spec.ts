...
/** Golden WU account-load-latest-export-job -- loadLatestExportJob. */
describe('AccountState loadLatestExportJob (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadLatestExportJob(); }).not.toThrow();
  });
});
