...
/** Golden WU account-ensure-loaded-for-section -- ensureLoadedForSection. */
describe('AccountState ensureLoadedForSection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).ensureLoadedForSection(undefined as any); }).not.toThrow();
  });
});
