...
/** Golden WU account-confirm-remove-passkey -- confirmRemovePasskey. */
describe('AccountState confirmRemovePasskey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).confirmRemovePasskey(); }).not.toThrow();
  });
});
