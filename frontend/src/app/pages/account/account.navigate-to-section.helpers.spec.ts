...
/** Golden WU account-navigate-to-section -- navigateToSection. */
describe('AccountState navigateToSection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).navigateToSection(''); }).not.toThrow();
  });
});
