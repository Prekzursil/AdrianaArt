...
/** Golden WU account-load-secondary-emails -- loadSecondaryEmails. */
describe('AccountState loadSecondaryEmails (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadSecondaryEmails(false); }).not.toThrow();
  });
});
