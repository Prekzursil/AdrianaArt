...
/** Golden WU account-apply-order-filters -- applyOrderFilters. */
describe('AccountState applyOrderFilters (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyOrderFilters(); }).not.toThrow();
  });
});
