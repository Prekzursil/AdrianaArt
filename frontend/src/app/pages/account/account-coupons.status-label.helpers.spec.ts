...
/** Golden WU account-coupons-status-label -- statusLabel. */
describe('AccountCouponsComponent statusLabel (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => (cmp as any).statusLabel({} as any)).not.toThrow();
  });
});
