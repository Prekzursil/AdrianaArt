...
/** Golden WU account-discard-address-changes -- discardAddressChanges. */
describe('AccountState discardAddressChanges (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).discardAddressChanges(); }).not.toThrow();
  });
});
