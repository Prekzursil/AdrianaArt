...
/** Golden WU account-stop-export-job-polling -- stopExportJobPolling. */
describe('AccountState stopExportJobPolling (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).stopExportJobPolling(); }).not.toThrow();
  });
});
