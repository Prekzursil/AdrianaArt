...
/** Golden WU account-order-status-chip-class -- orderStatusChipClass. */
describe('AccountState orderStatusChipClass (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).orderStatusChipClass(''); }).not.toThrow();
  });
});
