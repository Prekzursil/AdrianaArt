...
/** Golden WU account-load-two-factor-status -- loadTwoFactorStatus. */
describe('AccountState loadTwoFactorStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadTwoFactorStatus(false); }).not.toThrow();
  });
});
