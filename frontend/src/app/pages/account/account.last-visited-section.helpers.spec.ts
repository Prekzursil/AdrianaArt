...
/** Golden WU account-last-visited-section -- lastVisitedSection. */
describe('AccountState lastVisitedSection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).lastVisitedSection(); }).not.toThrow();
  });
});
