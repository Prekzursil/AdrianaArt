...
/** Golden WU account-wishlist-count-label -- wishlistCountLabel. */
describe('AccountState wishlistCountLabel (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).wishlistCountLabel(); }).not.toThrow();
  });
});
