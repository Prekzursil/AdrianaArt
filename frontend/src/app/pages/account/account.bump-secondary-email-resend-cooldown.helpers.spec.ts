...
/** Golden WU account-bump-secondary-email-resend-cooldown -- bumpSecondaryEmailResendCooldown. */
describe('AccountState bumpSecondaryEmailResendCooldown (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).bumpSecondaryEmailResendCooldown(''); }).not.toThrow();
  });
});
