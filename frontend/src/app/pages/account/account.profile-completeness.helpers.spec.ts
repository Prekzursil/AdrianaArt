...
/** Golden WU account-profile-completeness -- profileCompleteness. */
describe('AccountState profileCompleteness (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).profileCompleteness(); }).not.toThrow();
  });
});
