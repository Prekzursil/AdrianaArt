...
/** Golden WU page-load-legal-index-docs -- loadLegalIndexDocs. */
describe('CmsPageComponent loadLegalIndexDocs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadLegalIndexDocs('', ''); }).not.toThrow();
  });
});
