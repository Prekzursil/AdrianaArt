...
/** Golden WU admin-drag-event-has-files -- dragEventHasFiles. */
describe('AdminComponent dragEventHasFiles (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).dragEventHasFiles({} as any); }).not.toThrow();
  });
});
