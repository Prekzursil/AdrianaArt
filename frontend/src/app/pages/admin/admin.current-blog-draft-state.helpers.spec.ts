...
/** Golden WU admin-current-blog-draft-state -- currentBlogDraftState. */
describe('AdminComponent currentBlogDraftState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).currentBlogDraftState(); }).not.toThrow();
  });
});
