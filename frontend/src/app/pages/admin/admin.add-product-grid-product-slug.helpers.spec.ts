...
/** Golden WU admin-add-product-grid-product-slug -- addProductGridProductSlug. */
describe('AdminComponent addProductGridProductSlug (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).addProductGridProductSlug('', ''); }).not.toThrow();
  });
});
