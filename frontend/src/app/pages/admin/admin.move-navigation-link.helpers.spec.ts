...
/** Golden WU admin-move-navigation-link -- moveNavigationLink. */
describe('AdminComponent moveNavigationLink (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).moveNavigationLink(undefined as any, '', 0); }).not.toThrow();
  });
});
