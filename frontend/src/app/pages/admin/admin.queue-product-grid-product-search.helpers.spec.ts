...
/** Golden WU admin-queue-product-grid-product-search -- queueProductGridProductSearch. */
describe('AdminComponent queueProductGridProductSearch (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).queueProductGridProductSearch('', ''); }).not.toThrow();
  });
});
