...
/** Golden WU admin-returns-cmp-on-return-label-selected -- onReturnLabelSelected. */
describe('AdminReturnsComponent onReturnLabelSelected (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onReturnLabelSelected({} as any); }).not.toThrow();
  });
});
