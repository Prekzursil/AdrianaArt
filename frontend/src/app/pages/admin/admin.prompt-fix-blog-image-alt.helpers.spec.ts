...
/** Golden WU admin-prompt-fix-blog-image-alt -- promptFixBlogImageAlt. */
describe('AdminComponent promptFixBlogImageAlt (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).promptFixBlogImageAlt(0); }).not.toThrow();
  });
});
