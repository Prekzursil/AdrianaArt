...
/** Golden WU admin-set-image-block-url -- setImageBlockUrl. */
describe('AdminComponent setImageBlockUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setImageBlockUrl('', undefined as any); }).not.toThrow();
  });
});
