...
/** Golden WU admin-read-autosave-candidate -- readAutosaveCandidate. */
describe('AdminComponent readAutosaveCandidate (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).readAutosaveCandidate(''); }).not.toThrow();
  });
});
