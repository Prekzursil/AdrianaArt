...
/** Golden WU admin-load-blog-seo-snapshots -- loadBlogSeoSnapshots. */
describe('AdminComponent loadBlogSeoSnapshots (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadBlogSeoSnapshots(''); }).not.toThrow();
  });
});
