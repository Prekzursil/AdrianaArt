...
/** Golden WU admin-safe-record-key -- safeRecordKey. */
describe('AdminComponent safeRecordKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).safeRecordKey('', undefined as any); }).not.toThrow();
  });
});
