...
/** Golden WU admin-add-gallery-image-from-asset -- addGalleryImageFromAsset. */
describe('AdminComponent addGalleryImageFromAsset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).addGalleryImageFromAsset('', undefined as any); }).not.toThrow();
  });
});
