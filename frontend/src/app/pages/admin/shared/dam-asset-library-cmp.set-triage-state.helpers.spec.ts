...
/** Golden WU dam-asset-library-cmp-set-triage-state -- setTriageState. */
describe('DamAssetLibraryComponent setTriageState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setTriageState(undefined as any, undefined as any); }).not.toThrow();
  });
});
