...
/** Golden WU dam-asset-library-retry-job -- retryJob. */
describe('DamAssetLibraryComponent retryJob (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryJob(undefined as any); }).not.toThrow();
  });
});
