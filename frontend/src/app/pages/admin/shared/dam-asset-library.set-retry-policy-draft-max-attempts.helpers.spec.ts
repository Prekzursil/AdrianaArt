...
/** Golden WU dam-asset-library-set-retry-policy-draft-max-attempts -- setRetryPolicyDraftMaxAttempts. */
describe('DamAssetLibraryComponent setRetryPolicyDraftMaxAttempts (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setRetryPolicyDraftMaxAttempts(undefined as any, undefined as any); }).not.toThrow();
  });
});
