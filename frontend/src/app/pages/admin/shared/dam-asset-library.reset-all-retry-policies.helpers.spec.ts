...
/** Golden WU dam-asset-library-reset-all-retry-policies -- resetAllRetryPolicies. */
describe('DamAssetLibraryComponent resetAllRetryPolicies (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.resetAllRetryPolicies(); }).not.toThrow();
  });
});
