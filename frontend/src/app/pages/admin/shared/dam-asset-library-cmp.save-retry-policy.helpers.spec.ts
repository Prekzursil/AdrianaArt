...
/** Golden WU dam-asset-library-cmp-save-retry-policy -- saveRetryPolicy. */
describe('DamAssetLibraryComponent saveRetryPolicy (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).saveRetryPolicy(undefined as any); }).not.toThrow();
  });
});
