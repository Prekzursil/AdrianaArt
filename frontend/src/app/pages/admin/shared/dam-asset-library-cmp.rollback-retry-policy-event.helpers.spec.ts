...
/** Golden WU dam-asset-library-cmp-rollback-retry-policy-event -- rollbackRetryPolicyEvent. */
describe('DamAssetLibraryComponent rollbackRetryPolicyEvent (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).rollbackRetryPolicyEvent(undefined as any, ''); }).not.toThrow();
  });
});
