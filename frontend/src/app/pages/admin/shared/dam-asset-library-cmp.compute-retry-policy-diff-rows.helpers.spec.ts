...
/** Golden WU dam-asset-library-cmp-compute-retry-policy-diff-rows -- computeRetryPolicyDiffRows. */
describe('DamAssetLibraryComponent computeRetryPolicyDiffRows (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).computeRetryPolicyDiffRows(undefined as any, undefined as any); }).not.toThrow();
  });
});
