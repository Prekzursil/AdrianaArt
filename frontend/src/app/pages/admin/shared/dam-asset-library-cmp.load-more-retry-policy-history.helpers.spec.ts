...
/** Golden WU dam-asset-library-cmp-load-more-retry-policy-history -- loadMoreRetryPolicyHistory. */
describe('DamAssetLibraryComponent loadMoreRetryPolicyHistory (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadMoreRetryPolicyHistory(undefined as any); }).not.toThrow();
  });
});
