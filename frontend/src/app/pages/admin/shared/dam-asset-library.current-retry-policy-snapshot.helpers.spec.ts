...
/** Golden WU dam-asset-library-current-retry-policy-snapshot -- currentRetryPolicySnapshot. */
describe('DamAssetLibraryComponent currentRetryPolicySnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.currentRetryPolicySnapshot(undefined as any); }).not.toThrow();
  });
});
