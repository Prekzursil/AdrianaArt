...
/** Golden WU dam-asset-library-cmp-apply-retry-policy-saved-state -- applyRetryPolicySavedState. */
describe('DamAssetLibraryComponent applyRetryPolicySavedState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyRetryPolicySavedState(undefined as any); }).not.toThrow();
  });
});
