...
/** Golden WU dam-asset-library-mark-retry-policy-known-good -- markRetryPolicyKnownGood. */
describe('DamAssetLibraryComponent markRetryPolicyKnownGood (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).markRetryPolicyKnownGood(undefined as any); }).not.toThrow();
  });
});
