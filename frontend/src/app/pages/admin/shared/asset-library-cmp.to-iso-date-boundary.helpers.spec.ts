...
/** Golden WU asset-library-cmp-to-iso-date-boundary -- toIsoDateBoundary. */
describe('AssetLibraryComponent toIsoDateBoundary (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toIsoDateBoundary('', undefined as any); }).not.toThrow();
  });
});
