...
/** Golden WU content-revisions-cmp-rollback-selected -- rollbackSelected. */
describe('ContentRevisionsComponent rollbackSelected (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).rollbackSelected(); }).not.toThrow();
  });
});
