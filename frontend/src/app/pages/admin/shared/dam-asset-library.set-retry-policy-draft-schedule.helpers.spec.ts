...
/** Golden WU dam-asset-library-set-retry-policy-draft-schedule -- setRetryPolicyDraftSchedule. */
describe('DamAssetLibraryComponent setRetryPolicyDraftSchedule (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setRetryPolicyDraftSchedule(undefined as any, ''); }).not.toThrow();
  });
});
