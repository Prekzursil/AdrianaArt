...
/** Golden WU dam-asset-library-retry-policy-history-items -- retryPolicyHistoryItems. */
describe('DamAssetLibraryComponent retryPolicyHistoryItems (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryPolicyHistoryItems(undefined as any); }).not.toThrow();
  });
});
