...
/** Golden WU dam-asset-library-toggle-retry-policy-history -- toggleRetryPolicyHistory. */
describe('DamAssetLibraryComponent toggleRetryPolicyHistory (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toggleRetryPolicyHistory(undefined as any); }).not.toThrow();
  });
});
