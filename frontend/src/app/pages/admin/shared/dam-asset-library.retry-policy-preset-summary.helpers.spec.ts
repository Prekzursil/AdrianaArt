...
/** Golden WU dam-asset-library-retry-policy-preset-summary -- retryPolicyPresetSummary. */
describe('DamAssetLibraryComponent retryPolicyPresetSummary (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryPolicyPresetSummary(undefined as any); }).not.toThrow();
  });
});
