...
/** Golden WU dam-asset-library-retry-policy-history-loading -- retryPolicyHistoryLoading. */
describe('DamAssetLibraryComponent retryPolicyHistoryLoading (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryPolicyHistoryLoading(undefined as any); }).not.toThrow();
  });
});
