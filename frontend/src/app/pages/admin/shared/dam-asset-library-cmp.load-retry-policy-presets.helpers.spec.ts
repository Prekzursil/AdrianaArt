...
/** Golden WU dam-asset-library-cmp-load-retry-policy-presets -- loadRetryPolicyPresets. */
describe('DamAssetLibraryComponent loadRetryPolicyPresets (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadRetryPolicyPresets(undefined as any); }).not.toThrow();
  });
});
