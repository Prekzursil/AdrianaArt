...
/** Golden WU dam-asset-library-retry-policy-diff-chips -- retryPolicyDiffChips. */
describe('DamAssetLibraryComponent retryPolicyDiffChips (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryPolicyDiffChips(undefined as any, undefined as any); }).not.toThrow();
  });
});
