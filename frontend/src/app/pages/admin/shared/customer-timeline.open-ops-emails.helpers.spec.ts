...
/** Golden WU customer-timeline-open-ops-emails -- openOpsEmails. */
describe('CustomerTimelineComponent openOpsEmails (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openOpsEmails(); }).not.toThrow();
  });
});
