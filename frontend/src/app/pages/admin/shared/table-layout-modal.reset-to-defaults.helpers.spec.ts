...
/** Golden WU table-layout-modal-reset-to-defaults -- resetToDefaults. */
describe('TableLayoutModalComponent resetToDefaults (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resetToDefaults(); }).not.toThrow();
  });
});
