...
/** Golden WU dam-asset-library-retry-policy-history-error -- retryPolicyHistoryError. */
describe('DamAssetLibraryComponent retryPolicyHistoryError (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.retryPolicyHistoryError(undefined as any); }).not.toThrow();
  });
});
