...
/** Golden WU dam-asset-library-retry-policy-event-diff-rows -- retryPolicyEventDiffRows. */
describe('DamAssetLibraryComponent retryPolicyEventDiffRows (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.retryPolicyEventDiffRows({} as any); }).not.toThrow();
  });
});
