...
/** Golden WU dam-asset-library-bulk-remove-tag-from-selected-jobs -- bulkRemoveTagFromSelectedJobs. */
describe('DamAssetLibraryComponent bulkRemoveTagFromSelectedJobs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => (cmp as any).bulkRemoveTagFromSelectedJobs()).not.toThrow();
  });
});
