...
/** Golden WU dam-asset-library-cmp-reset-retry-policy -- resetRetryPolicy. */
describe('DamAssetLibraryComponent resetRetryPolicy (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resetRetryPolicy(undefined as any); }).not.toThrow();
  });
});
