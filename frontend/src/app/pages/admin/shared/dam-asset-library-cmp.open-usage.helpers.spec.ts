...
/** Golden WU dam-asset-library-cmp-open-usage -- openUsage. */
describe('DamAssetLibraryComponent openUsage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openUsage(undefined as any); }).not.toThrow();
  });
});
