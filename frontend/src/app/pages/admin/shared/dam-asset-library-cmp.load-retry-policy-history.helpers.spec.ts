...
/** Golden WU dam-asset-library-cmp-load-retry-policy-history -- loadRetryPolicyHistory. */
describe('DamAssetLibraryComponent loadRetryPolicyHistory (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadRetryPolicyHistory(undefined as any, false); }).not.toThrow();
  });
});
