...
/** Golden WU dam-asset-library-bulk-add-tag-to-selected-jobs -- bulkAddTagToSelectedJobs. */
describe('DamAssetLibraryComponent bulkAddTagToSelectedJobs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => (cmp as any).bulkAddTagToSelectedJobs()).not.toThrow();
  });
});
