...
/** Golden WU admin-find-replace-key-prefix -- findReplaceKeyPrefix. */
describe('AdminComponent findReplaceKeyPrefix (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).findReplaceKeyPrefix(); }).not.toThrow();
  });
});
