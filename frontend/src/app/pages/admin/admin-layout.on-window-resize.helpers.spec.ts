...
/** Golden WU admin-layout-on-window-resize -- onWindowResize. */
describe('AdminLayoutComponent onWindowResize (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onWindowResize(); }).not.toThrow();
  });
});
