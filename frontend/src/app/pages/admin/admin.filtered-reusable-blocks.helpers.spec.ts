...
/** Golden WU admin-filtered-reusable-blocks -- filteredReusableBlocks. */
describe('AdminComponent filteredReusableBlocks (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).filteredReusableBlocks(); }).not.toThrow();
  });
});
