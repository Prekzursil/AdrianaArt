...
/** Golden WU admin-has-restorable-autosave -- hasRestorableAutosave. */
describe('AdminComponent hasRestorableAutosave (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).hasRestorableAutosave(); }).not.toThrow();
  });
});
