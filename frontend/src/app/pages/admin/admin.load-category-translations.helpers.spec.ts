...
/** Golden WU admin-load-category-translations -- loadCategoryTranslations. */
describe('AdminComponent loadCategoryTranslations (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadCategoryTranslations(''); }).not.toThrow();
  });
});
