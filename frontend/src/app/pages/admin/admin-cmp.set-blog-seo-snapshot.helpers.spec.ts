...
/** Golden WU admin-cmp-set-blog-seo-snapshot -- setBlogSeoSnapshot. */
describe('AdminComponent setBlogSeoSnapshot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setBlogSeoSnapshot(undefined as any, '', ''); }).not.toThrow();
  });
});
