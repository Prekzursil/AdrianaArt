...
/** Golden WU admin-next-custom-block-key -- nextCustomBlockKey. */
describe('AdminComponent nextCustomBlockKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).nextCustomBlockKey(''); }).not.toThrow();
  });
});
