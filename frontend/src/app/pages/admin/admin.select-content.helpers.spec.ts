...
/** Golden WU admin-select-content -- selectContent. */
describe('AdminComponent selectContent (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).selectContent(undefined as any); }).not.toThrow();
  });
});
