...
/** Golden WU admin-remove-carousel-slide -- removeCarouselSlide. */
describe('AdminComponent removeCarouselSlide (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeCarouselSlide('', 0); }).not.toThrow();
  });
});
