...
/** Golden WU admin-on-navigation-drop -- onNavigationDrop. */
describe('AdminComponent onNavigationDrop (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onNavigationDrop(undefined as any, ''); }).not.toThrow();
  });
});
