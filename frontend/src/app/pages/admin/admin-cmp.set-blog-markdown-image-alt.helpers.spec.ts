...
/** Golden WU admin-cmp-set-blog-markdown-image-alt -- setBlogMarkdownImageAlt. */
describe('AdminComponent setBlogMarkdownImageAlt (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).setBlogMarkdownImageAlt(0, ''); }).not.toThrow();
  });
});
