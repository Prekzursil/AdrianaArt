...
/** Golden WU admin-reset-section-state -- resetSectionState. */
describe('AdminComponent resetSectionState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resetSectionState(undefined as any); }).not.toThrow();
  });
});
