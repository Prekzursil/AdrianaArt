...
/** Golden WU admin-load-for-section -- loadForSection. */
describe('AdminComponent loadForSection (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadForSection(undefined as any); }).not.toThrow();
  });
});
