...
/** Golden WU admin-on-blog-image-drag-over -- onBlogImageDragOver. */
describe('AdminComponent onBlogImageDragOver (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onBlogImageDragOver({} as any); }).not.toThrow();
  });
});
