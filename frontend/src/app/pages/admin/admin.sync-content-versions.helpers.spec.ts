...
/** Golden WU admin-sync-content-versions -- syncContentVersions. */
describe('AdminComponent syncContentVersions (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).syncContentVersions(undefined as any); }).not.toThrow();
  });
});
