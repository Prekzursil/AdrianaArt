...
/** Golden WU admin-support-cmp-toggle-template-active -- toggleTemplateActive. */
describe('AdminSupportComponent toggleTemplateActive (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toggleTemplateActive(undefined as any); }).not.toThrow();
  });
});
