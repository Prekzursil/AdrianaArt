...
/** Golden WU admin-support-cmp-load-assignees -- loadAssignees. */
describe('AdminSupportComponent loadAssignees (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadAssignees(); }).not.toThrow();
  });
});
