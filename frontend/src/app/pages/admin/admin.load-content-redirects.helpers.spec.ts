...
/** Golden WU admin-load-content-redirects -- loadContentRedirects. */
describe('AdminComponent loadContentRedirects (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadContentRedirects(false); }).not.toThrow();
  });
});
