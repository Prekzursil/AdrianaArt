...
/** Golden WU admin-load-flagged-comments -- loadFlaggedComments. */
describe('AdminComponent loadFlaggedComments (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadFlaggedComments(); }).not.toThrow();
  });
});
