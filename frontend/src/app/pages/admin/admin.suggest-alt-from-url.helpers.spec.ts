...
/** Golden WU admin-suggest-alt-from-url -- suggestAltFromUrl. */
describe('AdminComponent suggestAltFromUrl (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).suggestAltFromUrl(''); }).not.toThrow();
  });
});
