...
/** Golden WU admin-theme-cmp-evaluate-contrast -- evaluateContrast. */
describe('AdminThemeComponent evaluateContrast (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).evaluateContrast('', '', ''); }).not.toThrow();
  });
});
