...
/** Golden WU admin-init-from-server -- initFromServer. */
describe('AdminComponent initFromServer (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).initFromServer(undefined as any); }).not.toThrow();
  });
});
