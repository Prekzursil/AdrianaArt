...
/** Golden WU admin-on-show-hidden-pages-change -- onShowHiddenPagesChange. */
describe('AdminComponent onShowHiddenPagesChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onShowHiddenPagesChange(); }).not.toThrow();
  });
});
