...
/** Golden WU admin-change-order-status -- changeOrderStatus. */
describe('AdminComponent changeOrderStatus (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).changeOrderStatus(''); }).not.toThrow();
  });
});
