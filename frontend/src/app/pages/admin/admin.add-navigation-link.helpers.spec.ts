...
/** Golden WU admin-add-navigation-link -- addNavigationLink. */
describe('AdminComponent addNavigationLink (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).addNavigationLink(undefined as any); }).not.toThrow();
  });
});
