...
/** Golden WU admin-restore-autosave -- restoreAutosave. */
describe('AdminComponent restoreAutosave (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restoreAutosave(undefined as any); }).not.toThrow();
  });
});
