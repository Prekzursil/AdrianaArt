...
/** Golden WU admin-preview-origin-from-response -- previewOriginFromResponse. */
describe('AdminComponent previewOriginFromResponse (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).previewOriginFromResponse(undefined as any); }).not.toThrow();
  });
});
