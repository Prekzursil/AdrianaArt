...
/** Golden WU admin-serialize-slide-draft -- serializeSlideDraft. */
describe('AdminComponent serializeSlideDraft (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).serializeSlideDraft(undefined as any); }).not.toThrow();
  });
});
