...
/** Golden WU admin-default-cms-block-layout -- defaultCmsBlockLayout. */
describe('AdminComponent defaultCmsBlockLayout (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).defaultCmsBlockLayout(); }).not.toThrow();
  });
});
