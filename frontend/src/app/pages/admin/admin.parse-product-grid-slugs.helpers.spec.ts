...
/** Golden WU admin-parse-product-grid-slugs -- parseProductGridSlugs. */
describe('AdminComponent parseProductGridSlugs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).parseProductGridSlugs(''); }).not.toThrow();
  });
});
