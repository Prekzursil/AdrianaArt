...
/** Golden WU admin-ops-cmp-apply-email-failures-deep-link -- applyEmailFailuresDeepLink. */
describe('AdminOpsComponent applyEmailFailuresDeepLink (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyEmailFailuresDeepLink(); }).not.toThrow();
  });
});
