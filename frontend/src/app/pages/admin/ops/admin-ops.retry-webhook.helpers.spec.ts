...
/** Golden WU admin-ops-retry-webhook -- retryWebhook. */
describe('AdminOpsComponent retryWebhook (golden WU)', () => {
  it('returns early on guard', () => {
    ...
    expect(() => (cmp as any).retryWebhook({ target: null } as any)).not.toThrow();
  });
});
