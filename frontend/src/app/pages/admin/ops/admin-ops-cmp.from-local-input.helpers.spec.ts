...
/** Golden WU admin-ops-cmp-from-local-input -- fromLocalInput. */
describe('AdminOpsComponent fromLocalInput (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).fromLocalInput(''); }).not.toThrow();
  });
});
