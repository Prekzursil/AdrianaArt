...
/** Golden WU admin-ops-cmp-load-shipping-methods -- loadShippingMethods. */
describe('AdminOpsComponent loadShippingMethods (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadShippingMethods(); }).not.toThrow();
  });
});
