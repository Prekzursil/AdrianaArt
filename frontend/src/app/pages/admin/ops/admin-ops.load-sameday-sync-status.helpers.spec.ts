...
/** Golden WU admin-ops-load-sameday-sync-status -- loadSamedaySyncStatus. */
describe('AdminOpsComponent loadSamedaySyncStatus (golden WU)', () => {
  it('returns early on guard', () => {
    ...
    expect(() => (cmp as any).loadSamedaySyncStatus()).not.toThrow();
  });
});
