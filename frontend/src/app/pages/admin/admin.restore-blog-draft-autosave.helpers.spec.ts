...
/** Golden WU admin-restore-blog-draft-autosave -- restoreBlogDraftAutosave. */
describe('AdminComponent restoreBlogDraftAutosave (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restoreBlogDraftAutosave(); }).not.toThrow();
  });
});
