...
/** Golden WU admin-preview-find-replace -- previewFindReplace. */
describe('AdminComponent previewFindReplace (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).previewFindReplace(); }).not.toThrow();
  });
});
