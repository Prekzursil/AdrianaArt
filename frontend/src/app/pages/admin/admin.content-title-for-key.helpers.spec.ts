...
/** Golden WU admin-content-title-for-key -- contentTitleForKey. */
describe('AdminComponent contentTitleForKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).contentTitleForKey(''); }).not.toThrow();
  });
});
