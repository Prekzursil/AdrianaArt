...
/** Golden WU admin-to-localized-text -- toLocalizedText. */
describe('AdminComponent toLocalizedText (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toLocalizedText(undefined as any); }).not.toThrow();
  });
});
