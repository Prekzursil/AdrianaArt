...
/** Golden WU admin-load-reusable-blocks -- loadReusableBlocks. */
describe('AdminComponent loadReusableBlocks (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadReusableBlocks(); }).not.toThrow();
  });
});
