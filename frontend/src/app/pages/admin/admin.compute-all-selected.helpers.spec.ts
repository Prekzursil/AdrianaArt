...
/** Golden WU admin-compute-all-selected -- computeAllSelected. */
describe('AdminComponent computeAllSelected (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).computeAllSelected(); }).not.toThrow();
  });
});
