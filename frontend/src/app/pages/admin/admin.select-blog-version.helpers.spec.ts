...
/** Golden WU admin-select-blog-version -- selectBlogVersion. */
describe('AdminComponent selectBlogVersion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).selectBlogVersion(0); }).not.toThrow();
  });
});
