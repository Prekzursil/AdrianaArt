...
/** Golden WU admin-insert-at-cursor -- insertAtCursor. */
describe('AdminComponent insertAtCursor (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).insertAtCursor(undefined as any, ''); }).not.toThrow();
  });
});
