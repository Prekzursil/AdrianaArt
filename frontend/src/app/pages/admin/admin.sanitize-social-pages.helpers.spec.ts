...
/** Golden WU admin-sanitize-social-pages -- sanitizeSocialPages. */
describe('AdminComponent sanitizeSocialPages (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).sanitizeSocialPages(''); }).not.toThrow();
  });
});
