...
/** Golden WU admin-coupons-select-ab-coupon-b -- selectAbCouponB. */
describe('AdminCouponsComponent selectAbCouponB (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => (cmp as any).selectAbCouponB({ code: "X", promotion: null } as any)).not.toThrow();
  });
});
