...
/** Golden WU admin-coupons-cmp-load-coupons-after-mutation -- loadCouponsAfterMutation. */
describe('AdminCouponsComponent loadCouponsAfterMutation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadCouponsAfterMutation(undefined as any); }).not.toThrow();
  });
});
