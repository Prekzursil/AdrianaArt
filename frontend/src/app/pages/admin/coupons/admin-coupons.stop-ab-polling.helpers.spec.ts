...
/** Golden WU admin-coupons-stop-ab-polling -- stopAbPolling. */
describe('AdminCouponsComponent stopAbPolling (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).stopAbPolling(); }).not.toThrow();
  });
});
