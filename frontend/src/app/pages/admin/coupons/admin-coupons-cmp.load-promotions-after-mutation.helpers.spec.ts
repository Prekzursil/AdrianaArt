...
/** Golden WU admin-coupons-cmp-load-promotions-after-mutation -- loadPromotionsAfterMutation. */
describe('AdminCouponsComponent loadPromotionsAfterMutation (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadPromotionsAfterMutation(undefined as any); }).not.toThrow();
  });
});
