...
/** Golden WU admin-coupons-cmp-load-scoped-products -- loadScopedProducts. */
describe('AdminCouponsComponent loadScopedProducts (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadScopedProducts(); }).not.toThrow();
  });
});
