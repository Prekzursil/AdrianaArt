...
/** Golden WU admin-coupons-on-bulk-file-change -- onBulkFileChange. */
describe('AdminCouponsComponent onBulkFileChange (golden WU)', () => {
  it('returns early on guard', () => {
    ...
    expect(() => (cmp as any).onBulkFileChange({ target: { checked: false } } as any)).not.toThrow();
  });
});
