...
/** Golden WU admin-coupons-parse-emails-from-csv -- parseEmailsFromCsv. */
describe('AdminCouponsComponent parseEmailsFromCsv (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).parseEmailsFromCsv(''); }).not.toThrow();
  });
});
