...
/** Golden WU admin-coupons-cmp-refresh-segment-job -- refreshSegmentJob. */
describe('AdminCouponsComponent refreshSegmentJob (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).refreshSegmentJob(''); }).not.toThrow();
  });
});
