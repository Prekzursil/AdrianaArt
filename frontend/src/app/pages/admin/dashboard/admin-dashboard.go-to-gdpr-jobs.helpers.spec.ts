...
/** Golden WU admin-dashboard-go-to-gdpr-jobs -- goToGdprJobs. */
describe('AdminDashboardComponent goToGdprJobs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).goToGdprJobs(); }).not.toThrow();
  });
});
