...
/** Golden WU admin-dashboard-open-failed-payments -- openFailedPayments. */
describe('AdminDashboardComponent openFailedPayments (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openFailedPayments(); }).not.toThrow();
  });
});
