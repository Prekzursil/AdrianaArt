...
/** Golden WU admin-dashboard-load-funnel-metrics -- loadFunnelMetrics. */
describe('AdminDashboardComponent loadFunnelMetrics (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadFunnelMetrics(); }).not.toThrow();
  });
});
