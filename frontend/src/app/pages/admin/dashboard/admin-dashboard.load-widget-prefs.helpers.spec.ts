...
/** Golden WU admin-dashboard-load-widget-prefs -- loadWidgetPrefs. */
describe('AdminDashboardComponent loadWidgetPrefs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadWidgetPrefs(); }).not.toThrow();
  });
});
