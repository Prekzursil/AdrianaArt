...
/** Golden WU admin-dashboard-start-live-refresh -- startLiveRefresh. */
describe('AdminDashboardComponent startLiveRefresh (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).startLiveRefresh(); }).not.toThrow();
  });
});
