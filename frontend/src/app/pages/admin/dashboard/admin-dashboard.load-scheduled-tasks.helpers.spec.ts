...
/** Golden WU admin-dashboard-load-scheduled-tasks -- loadScheduledTasks. */
describe('AdminDashboardComponent loadScheduledTasks (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadScheduledTasks(); }).not.toThrow();
  });
});
