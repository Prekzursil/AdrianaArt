...
/** Golden WU admin-dashboard-parse-required-count -- parseRequiredCount. */
describe('AdminDashboardComponent parseRequiredCount (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).parseRequiredCount(undefined as any); }).not.toThrow();
  });
});
