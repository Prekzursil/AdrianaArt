...
/** Golden WU admin-dashboard-on-global-search-keydown -- onGlobalSearchKeydown. */
describe('AdminDashboardComponent onGlobalSearchKeydown (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onGlobalSearchKeydown({} as any); }).not.toThrow();
  });
});
