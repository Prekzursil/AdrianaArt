...
/** Golden WU admin-dashboard-run-global-search -- runGlobalSearch. */
describe('AdminDashboardComponent runGlobalSearch (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).runGlobalSearch(''); }).not.toThrow();
  });
});
