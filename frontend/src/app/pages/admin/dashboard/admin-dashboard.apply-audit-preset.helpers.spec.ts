...
/** Golden WU admin-dashboard-apply-audit-preset -- applyAuditPreset. */
describe('AdminDashboardComponent applyAuditPreset (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyAuditPreset(undefined as any); }).not.toThrow();
  });
});
