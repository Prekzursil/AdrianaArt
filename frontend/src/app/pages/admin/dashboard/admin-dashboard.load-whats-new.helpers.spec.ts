...
/** Golden WU admin-dashboard-load-whats-new -- loadWhatsNew. */
describe('AdminDashboardComponent loadWhatsNew (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadWhatsNew(undefined as any); }).not.toThrow();
  });
});
