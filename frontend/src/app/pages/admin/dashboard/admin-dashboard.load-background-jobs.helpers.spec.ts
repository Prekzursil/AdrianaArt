...
/** Golden WU admin-dashboard-load-background-jobs -- loadBackgroundJobs. */
describe('AdminDashboardComponent loadBackgroundJobs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadBackgroundJobs(); }).not.toThrow();
  });
});
