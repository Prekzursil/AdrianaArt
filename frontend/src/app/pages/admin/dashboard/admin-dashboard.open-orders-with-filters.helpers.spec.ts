...
/** Golden WU admin-dashboard-open-orders-with-filters -- openOrdersWithFilters. */
describe('AdminDashboardComponent openOrdersWithFilters (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openOrdersWithFilters(undefined as any); }).not.toThrow();
  });
});
