...
/** Golden WU admin-dashboard-load-stockout-impact -- loadStockoutImpact. */
describe('AdminDashboardComponent loadStockoutImpact (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadStockoutImpact(); }).not.toThrow();
  });
});
