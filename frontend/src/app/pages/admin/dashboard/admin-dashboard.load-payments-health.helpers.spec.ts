...
/** Golden WU admin-dashboard-load-payments-health -- loadPaymentsHealth. */
describe('AdminDashboardComponent loadPaymentsHealth (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadPaymentsHealth(); }).not.toThrow();
  });
});
