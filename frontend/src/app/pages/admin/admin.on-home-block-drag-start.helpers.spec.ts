...
/** Golden WU admin-on-home-block-drag-start -- onHomeBlockDragStart. */
describe('AdminComponent onHomeBlockDragStart (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onHomeBlockDragStart(''); }).not.toThrow();
  });
});
