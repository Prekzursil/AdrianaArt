...
/** Golden WU admin-read-cms-block-payload -- readCmsBlockPayload. */
describe('AdminComponent readCmsBlockPayload (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).readCmsBlockPayload({} as any); }).not.toThrow();
  });
});
