...
/** Golden WU admin-remove-product-grid-product-slug -- removeProductGridProductSlug. */
describe('AdminComponent removeProductGridProductSlug (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeProductGridProductSlug('', ''); }).not.toThrow();
  });
});
