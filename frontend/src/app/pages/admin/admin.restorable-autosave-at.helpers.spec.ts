...
/** Golden WU admin-restorable-autosave-at -- restorableAutosaveAt. */
describe('AdminComponent restorableAutosaveAt (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restorableAutosaveAt(); }).not.toThrow();
  });
});
