...
/** Golden WU admin-save-checkout-settings -- saveCheckoutSettings. */
describe('AdminComponent saveCheckoutSettings (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).saveCheckoutSettings(); }).not.toThrow();
  });
});
