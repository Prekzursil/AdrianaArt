...
/** Golden WU admin-users-revoke-one-session -- revokeOneSession. */
describe('AdminUsersComponent revokeOneSession (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).revokeOneSession(''); }).not.toThrow();
  });
});
