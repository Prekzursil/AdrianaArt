...
/** Golden WU admin-cmp-apply-page-draft-state -- applyPageDraftState. */
describe('AdminComponent applyPageDraftState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyPageDraftState(undefined as any, undefined as any); }).not.toThrow();
  });
});
