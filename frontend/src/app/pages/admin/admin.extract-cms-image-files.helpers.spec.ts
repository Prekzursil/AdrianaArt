...
/** Golden WU admin-extract-cms-image-files -- extractCmsImageFiles. */
describe('AdminComponent extractCmsImageFiles (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).extractCmsImageFiles({} as any); }).not.toThrow();
  });
});
