...
/** Golden WU admin-remove-gallery-image -- removeGalleryImage. */
describe('AdminComponent removeGalleryImage (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeGalleryImage('', 0); }).not.toThrow();
  });
});
