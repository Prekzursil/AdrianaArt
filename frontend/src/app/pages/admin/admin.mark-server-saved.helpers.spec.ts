...
/** Golden WU admin-mark-server-saved -- markServerSaved. */
describe('AdminComponent markServerSaved (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).markServerSaved(undefined as any, undefined as any); }).not.toThrow();
  });
});
