...
/** Golden WU admin-reset-navigation-defaults -- resetNavigationDefaults. */
describe('AdminComponent resetNavigationDefaults (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resetNavigationDefaults(); }).not.toThrow();
  });
});
