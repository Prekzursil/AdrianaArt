...
/** Golden WU admin-load-checkout-settings -- loadCheckoutSettings. */
describe('AdminComponent loadCheckoutSettings (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadCheckoutSettings(); }).not.toThrow();
  });
});
