...
/** Golden WU admin-order-detail-event-diff-rows -- eventDiffRows. */
describe('AdminOrderDetailComponent eventDiffRows (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).eventDiffRows({} as any); }).not.toThrow();
  });
});
