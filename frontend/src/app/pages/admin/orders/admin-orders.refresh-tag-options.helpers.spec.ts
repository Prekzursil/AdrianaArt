...
/** Golden WU admin-orders-refresh-tag-options -- refreshTagOptions. */
describe('AdminOrdersComponent refreshTagOptions (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).refreshTagOptions(); }).not.toThrow();
  });
});
