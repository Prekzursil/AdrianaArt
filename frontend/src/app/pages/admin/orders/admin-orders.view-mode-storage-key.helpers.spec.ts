...
/** Golden WU admin-orders-view-mode-storage-key -- viewModeStorageKey. */
describe('AdminOrdersComponent viewModeStorageKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).viewModeStorageKey(); }).not.toThrow();
  });
});
