...
/** Golden WU admin-order-detail-apply-nav-context -- applyNavContext. */
describe('AdminOrderDetailComponent applyNavContext (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyNavContext(undefined as any); }).not.toThrow();
  });
});
