...
/** Golden WU admin-orders-resend-bulk-emails -- resendBulkEmails. */
describe('AdminOrdersComponent resendBulkEmails (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).resendBulkEmails(); }).not.toThrow();
  });
});
