...
/** Golden WU admin-orders-on-shipping-labels-selected -- onShippingLabelsSelected. */
describe('AdminOrdersComponent onShippingLabelsSelected (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onShippingLabelsSelected({} as any); }).not.toThrow();
  });
});
