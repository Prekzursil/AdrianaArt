...
/** Golden WU admin-order-detail-on-status-value-change -- onStatusValueChange. */
describe('AdminOrderDetailComponent onStatusValueChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onStatusValueChange(undefined as any); }).not.toThrow();
  });
});
