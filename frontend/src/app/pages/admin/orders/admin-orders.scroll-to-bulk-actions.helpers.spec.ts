...
/** Golden WU admin-orders-scroll-to-bulk-actions -- scrollToBulkActions. */
describe('AdminOrdersComponent scrollToBulkActions (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).scrollToBulkActions(); }).not.toThrow();
  });
});
