...
/** Golden WU admin-order-detail-open-shipment-editor -- openShipmentEditor. */
describe('AdminOrderDetailComponent openShipmentEditor (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).openShipmentEditor(undefined as any); }).not.toThrow();
  });
});
