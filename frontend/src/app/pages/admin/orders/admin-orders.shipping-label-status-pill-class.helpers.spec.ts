...
/** Golden WU admin-orders-shipping-label-status-pill-class -- shippingLabelStatusPillClass. */
describe('AdminOrdersComponent shippingLabelStatusPillClass (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).shippingLabelStatusPillClass(undefined as any); }).not.toThrow();
  });
});
