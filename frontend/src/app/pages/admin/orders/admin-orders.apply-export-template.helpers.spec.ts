...
/** Golden WU admin-orders-apply-export-template -- applyExportTemplate. */
describe('AdminOrdersComponent applyExportTemplate (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyExportTemplate(''); }).not.toThrow();
  });
});
