...
/** Golden WU admin-order-detail-on-document-keydown -- onDocumentKeydown. */
describe('AdminOrderDetailComponent onDocumentKeydown (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onDocumentKeydown({} as any); }).not.toThrow();
  });
});
