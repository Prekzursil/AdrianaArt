...
/** Golden WU admin-orders-shipping-label-status-label-key -- shippingLabelStatusLabelKey. */
describe('AdminOrdersComponent shippingLabelStatusLabelKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).shippingLabelStatusLabelKey(undefined as any); }).not.toThrow();
  });
});
