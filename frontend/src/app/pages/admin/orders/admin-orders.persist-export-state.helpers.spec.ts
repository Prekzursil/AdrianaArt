...
/** Golden WU admin-orders-persist-export-state -- persistExportState. */
describe('AdminOrdersComponent persistExportState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistExportState(); }).not.toThrow();
  });
});
