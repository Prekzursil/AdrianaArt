...
/** Golden WU admin-order-detail-on-shipping-label-selected -- onShippingLabelSelected. */
describe('AdminOrderDetailComponent onShippingLabelSelected (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onShippingLabelSelected({} as any); }).not.toThrow();
  });
});
