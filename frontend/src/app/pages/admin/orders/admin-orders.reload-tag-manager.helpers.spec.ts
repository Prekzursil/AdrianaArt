...
/** Golden WU admin-orders-reload-tag-manager -- reloadTagManager. */
describe('AdminOrdersComponent reloadTagManager (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).reloadTagManager(); }).not.toThrow();
  });
});
