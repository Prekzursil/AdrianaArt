...
/** Golden WU admin-order-detail-event-address-diff -- eventAddressDiff. */
describe('AdminOrderDetailComponent eventAddressDiff (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).eventAddressDiff({} as any); }).not.toThrow();
  });
});
