...
/** Golden WU admin-orders-retry-shipping-label-upload -- retryShippingLabelUpload. */
describe('AdminOrdersComponent retryShippingLabelUpload (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).retryShippingLabelUpload(0); }).not.toThrow();
  });
});
