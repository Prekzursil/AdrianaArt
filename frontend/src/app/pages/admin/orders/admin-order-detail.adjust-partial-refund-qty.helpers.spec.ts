...
/** Golden WU admin-order-detail-adjust-partial-refund-qty -- adjustPartialRefundQty. */
describe('AdminOrderDetailComponent adjustPartialRefundQty (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).adjustPartialRefundQty('', 0, 0); }).not.toThrow();
  });
});
