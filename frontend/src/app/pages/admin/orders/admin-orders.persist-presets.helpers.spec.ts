...
/** Golden WU admin-orders-persist-presets -- persistPresets. */
describe('AdminOrdersComponent persistPresets (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).persistPresets(); }).not.toThrow();
  });
});
