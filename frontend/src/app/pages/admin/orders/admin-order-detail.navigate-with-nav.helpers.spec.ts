...
/** Golden WU admin-order-detail-navigate-with-nav -- navigateWithNav. */
describe('AdminOrderDetailComponent navigateWithNav (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).navigateWithNav('', 0); }).not.toThrow();
  });
});
