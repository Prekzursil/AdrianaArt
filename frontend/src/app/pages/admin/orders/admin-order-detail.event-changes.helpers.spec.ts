...
/** Golden WU admin-order-detail-event-changes -- eventChanges. */
describe('AdminOrderDetailComponent eventChanges (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).eventChanges({} as any); }).not.toThrow();
  });
});
