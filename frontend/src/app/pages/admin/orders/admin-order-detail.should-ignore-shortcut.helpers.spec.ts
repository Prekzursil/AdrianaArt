...
/** Golden WU admin-order-detail-should-ignore-shortcut -- shouldIgnoreShortcut. */
describe('AdminOrderDetailComponent shouldIgnoreShortcut (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).shouldIgnoreShortcut({} as any); }).not.toThrow();
  });
});
