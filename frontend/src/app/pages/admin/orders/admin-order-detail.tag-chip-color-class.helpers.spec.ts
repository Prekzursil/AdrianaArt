...
/** Golden WU admin-order-detail-tag-chip-color-class -- tagChipColorClass. */
describe('AdminOrderDetailComponent tagChipColorClass (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).tagChipColorClass(''); }).not.toThrow();
  });
});
