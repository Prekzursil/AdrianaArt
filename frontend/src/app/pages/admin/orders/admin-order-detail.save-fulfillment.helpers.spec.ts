...
/** Golden WU admin-order-detail-save-fulfillment -- saveFulfillment. */
describe('AdminOrderDetailComponent saveFulfillment (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).saveFulfillment('', 0); }).not.toThrow();
  });
});
