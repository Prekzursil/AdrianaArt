...
/** Golden WU admin-orders-filtered-tag-manager-rows -- filteredTagManagerRows. */
describe('AdminOrdersComponent filteredTagManagerRows (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { cmp.filteredTagManagerRows(); }).not.toThrow();
  });
});
