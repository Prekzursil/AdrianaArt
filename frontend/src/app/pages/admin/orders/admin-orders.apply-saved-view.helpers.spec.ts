...
/** Golden WU admin-orders-apply-saved-view -- applySavedView. */
describe('AdminOrdersComponent applySavedView (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applySavedView(''); }).not.toThrow();
  });
});
