...
/** Golden WU admin-order-detail-partial-refund-already-refunded-qty -- partialRefundAlreadyRefundedQty. */
describe('AdminOrderDetailComponent partialRefundAlreadyRefundedQty (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).partialRefundAlreadyRefundedQty(''); }).not.toThrow();
  });
});
