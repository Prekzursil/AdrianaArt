...
/** Golden WU admin-orders-save-export-template -- saveExportTemplate. */
describe('AdminOrdersComponent saveExportTemplate (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).saveExportTemplate(); }).not.toThrow();
  });
});
