...
/** Golden WU admin-on-blog-pin-drag-start -- onBlogPinDragStart. */
describe('AdminComponent onBlogPinDragStart (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onBlogPinDragStart(''); }).not.toThrow();
  });
});
