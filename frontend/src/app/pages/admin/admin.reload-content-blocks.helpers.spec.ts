...
/** Golden WU admin-reload-content-blocks -- reloadContentBlocks. */
describe('AdminComponent reloadContentBlocks (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).reloadContentBlocks(); }).not.toThrow();
  });
});
