...
/** Golden WU admin-slugify-reusable-block-id -- slugifyReusableBlockId. */
describe('AdminComponent slugifyReusableBlockId (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).slugifyReusableBlockId(''); }).not.toThrow();
  });
});
