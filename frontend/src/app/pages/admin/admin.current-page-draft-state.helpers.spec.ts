...
/** Golden WU admin-current-page-draft-state -- currentPageDraftState. */
describe('AdminComponent currentPageDraftState (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).currentPageDraftState(undefined as any); }).not.toThrow();
  });
});
