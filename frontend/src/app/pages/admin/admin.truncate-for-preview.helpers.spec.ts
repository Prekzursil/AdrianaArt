...
/** Golden WU admin-truncate-for-preview -- truncateForPreview. */
describe('AdminComponent truncateForPreview (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).truncateForPreview('', 0); }).not.toThrow();
  });
});
