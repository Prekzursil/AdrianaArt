...
/** Golden WU admin-remove-home-faq-item -- removeHomeFaqItem. */
describe('AdminComponent removeHomeFaqItem (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeHomeFaqItem('', 0); }).not.toThrow();
  });
});
