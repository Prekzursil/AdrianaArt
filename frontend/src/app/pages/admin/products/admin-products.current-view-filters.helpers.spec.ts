...
/** Golden WU admin-products-current-view-filters -- currentViewFilters. */
describe('AdminProductsComponent currentViewFilters (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).currentViewFilters(); }).not.toThrow();
  });
});
