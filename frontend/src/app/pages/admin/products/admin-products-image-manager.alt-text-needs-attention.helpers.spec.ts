...
/** Golden WU admin-products-image-manager-alt-text-needs-attention -- altTextNeedsAttention. */
describe('AdminProductsImageManagerComponent altTextNeedsAttention (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    const cmp = Object.create(AdminProductsImageManagerComponent.prototype) as AdminProductsImageManagerComponent;
    ...
    expect(() => { (cmp as any).altTextNeedsAttention(undefined as any); }).not.toThrow();
  });
});
