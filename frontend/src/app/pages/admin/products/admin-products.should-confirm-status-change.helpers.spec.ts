...
/** Golden WU admin-products-should-confirm-status-change -- shouldConfirmStatusChange. */
describe('AdminProductsComponent shouldConfirmStatusChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).shouldConfirmStatusChange(); }).not.toThrow();
  });
});
