...
/** Golden WU admin-products-run-product-search -- runProductSearch. */
describe('AdminProductsComponent runProductSearch (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).runProductSearch(''); }).not.toThrow();
  });
});
