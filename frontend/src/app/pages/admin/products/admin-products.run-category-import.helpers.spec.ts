...
/** Golden WU admin-products-run-category-import -- runCategoryImport. */
describe('AdminProductsComponent runCategoryImport (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).runCategoryImport(); }).not.toThrow();
  });
});
