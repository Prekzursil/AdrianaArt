...
/** Golden WU admin-products-table-layout-storage-key -- tableLayoutStorageKey. */
describe('AdminProductsComponent tableLayoutStorageKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).tableLayoutStorageKey(); }).not.toThrow();
  });
});
