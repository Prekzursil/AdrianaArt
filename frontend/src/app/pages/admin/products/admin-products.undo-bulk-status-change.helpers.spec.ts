...
/** Golden WU admin-products-undo-bulk-status-change -- undoBulkStatusChange. */
describe('AdminProductsComponent undoBulkStatusChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).undoBulkStatusChange(''); }).not.toThrow();
  });
});
