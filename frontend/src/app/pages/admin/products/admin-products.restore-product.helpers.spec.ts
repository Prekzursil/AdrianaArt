...
/** Golden WU admin-products-restore-product -- restoreProduct. */
describe('AdminProductsComponent restoreProduct (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).restoreProduct(undefined as any); }).not.toThrow();
  });
});
