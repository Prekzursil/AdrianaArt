...
/** Golden WU admin-products-on-csv-import-file-change -- onCsvImportFileChange. */
describe('AdminProductsComponent onCsvImportFileChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onCsvImportFileChange({} as any); }).not.toThrow();
  });
});
