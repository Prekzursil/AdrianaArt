...
/** Golden WU admin-products-run-duplicate-check -- runDuplicateCheck. */
describe('AdminProductsComponent runDuplicateCheck (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).runDuplicateCheck(); }).not.toThrow();
  });
});
