...
/** Golden WU admin-products-category-descendant-ids -- categoryDescendantIds. */
describe('AdminProductsComponent categoryDescendantIds (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).categoryDescendantIds(''); }).not.toThrow();
  });
});
