...
/** Golden WU admin-products-run-csv-import -- runCsvImport. */
describe('AdminProductsComponent runCsvImport (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).runCsvImport(false); }).not.toThrow();
  });
});
