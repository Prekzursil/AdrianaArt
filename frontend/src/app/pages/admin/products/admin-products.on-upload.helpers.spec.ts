...
/** Golden WU admin-products-on-upload -- onUpload. */
describe('AdminProductsComponent onUpload (golden WU)', () => {
  it('returns early on guard', () => {
    ...
    expect(() => (cmp as any).onUpload({ target: { checked: false } } as any)).not.toThrow();
  });
});
