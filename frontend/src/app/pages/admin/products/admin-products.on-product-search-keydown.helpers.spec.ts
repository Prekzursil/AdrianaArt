...
/** Golden WU admin-products-on-product-search-keydown -- onProductSearchKeydown. */
describe('AdminProductsComponent onProductSearchKeydown (golden WU)', () => {
  it('returns early on guard', () => {
    ...
    expect(() => (cmp as any).onProductSearchKeydown({ target: { checked: false } } as any)).not.toThrow();
  });
});
