...
/** Golden WU admin-products-merge-reason-key -- mergeReasonKey. */
describe('AdminProductsComponent mergeReasonKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).mergeReasonKey(undefined as any); }).not.toThrow();
  });
});
