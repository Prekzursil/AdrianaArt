...
/** Golden WU admin-products-on-variant-name-change -- onVariantNameChange. */
describe('AdminProductsComponent onVariantNameChange (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onVariantNameChange(0, ''); }).not.toThrow();
  });
});
