...
/** Golden WU admin-empty-localized-text -- emptyLocalizedText. */
describe('AdminComponent emptyLocalizedText (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).emptyLocalizedText(); }).not.toThrow();
  });
});
