...
/** Golden WU admin-on-blog-pin-drag-over -- onBlogPinDragOver. */
describe('AdminComponent onBlogPinDragOver (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onBlogPinDragOver({} as any); }).not.toThrow();
  });
});
