...
/** Golden WU admin-remove-social-link -- removeSocialLink. */
describe('AdminComponent removeSocialLink (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeSocialLink(undefined as any, 0); }).not.toThrow();
  });
});
