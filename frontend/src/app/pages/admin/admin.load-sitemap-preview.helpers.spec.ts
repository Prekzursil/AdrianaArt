...
/** Golden WU admin-load-sitemap-preview -- loadSitemapPreview. */
describe('AdminComponent loadSitemapPreview (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).loadSitemapPreview(); }).not.toThrow();
  });
});
