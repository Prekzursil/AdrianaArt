...
/** Golden WU admin-toggle-page-hidden -- togglePageHidden. */
describe('AdminComponent togglePageHidden (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).togglePageHidden(undefined as any); }).not.toThrow();
  });
});
