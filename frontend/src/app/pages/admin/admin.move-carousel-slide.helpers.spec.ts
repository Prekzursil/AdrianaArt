...
/** Golden WU admin-move-carousel-slide -- moveCarouselSlide. */
describe('AdminComponent moveCarouselSlide (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).moveCarouselSlide('', 0, 0); }).not.toThrow();
  });
});
