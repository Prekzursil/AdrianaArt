...
/** Golden WU admin-remove-navigation-link -- removeNavigationLink. */
describe('AdminComponent removeNavigationLink (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).removeNavigationLink(undefined as any, ''); }).not.toThrow();
  });
});
