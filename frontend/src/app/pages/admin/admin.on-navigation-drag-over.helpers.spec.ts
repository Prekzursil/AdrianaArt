...
/** Golden WU admin-on-navigation-drag-over -- onNavigationDragOver. */
describe('AdminComponent onNavigationDragOver (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onNavigationDragOver({} as any); }).not.toThrow();
  });
});
