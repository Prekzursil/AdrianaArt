...
/** Golden WU admin-toggle-select-all-blogs -- toggleSelectAllBlogs. */
describe('AdminComponent toggleSelectAllBlogs (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).toggleSelectAllBlogs({} as any); }).not.toThrow();
  });
});
