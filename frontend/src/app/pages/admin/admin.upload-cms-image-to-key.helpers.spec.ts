...
/** Golden WU admin-upload-cms-image-to-key -- uploadCmsImageToKey. */
describe('AdminComponent uploadCmsImageToKey (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).uploadCmsImageToKey('', undefined as any); }).not.toThrow();
  });
});
