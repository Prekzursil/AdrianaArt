...
/** Golden WU admin-blog-pinned-slot -- blogPinnedSlot. */
describe('AdminComponent blogPinnedSlot (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).blogPinnedSlot(undefined as any); }).not.toThrow();
  });
});
