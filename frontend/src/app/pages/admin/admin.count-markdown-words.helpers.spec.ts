...
/** Golden WU admin-count-markdown-words -- countMarkdownWords. */
describe('AdminComponent countMarkdownWords (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).countMarkdownWords(''); }).not.toThrow();
  });
});
