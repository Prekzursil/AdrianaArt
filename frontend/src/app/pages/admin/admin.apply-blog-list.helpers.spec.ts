...
/** Golden WU admin-apply-blog-list -- applyBlogList. */
describe('AdminComponent applyBlogList (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).applyBlogList(undefined as any); }).not.toThrow();
  });
});
