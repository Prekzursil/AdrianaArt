...
/** Golden WU admin-ensure-blog-draft -- ensureBlogDraft. */
describe('AdminComponent ensureBlogDraft (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).ensureBlogDraft('', undefined as any); }).not.toThrow();
  });
});
