...
/** Golden WU admin-on-category-drag-start -- onCategoryDragStart. */
describe('AdminComponent onCategoryDragStart (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).onCategoryDragStart(''); }).not.toThrow();
  });
});
