...
/** Golden WU admin-rollback-blog-version -- rollbackBlogVersion. */
describe('AdminComponent rollbackBlogVersion (golden WU)', () => {
  it('invokes without throwing when dependencies are stubbed', () => {
    ...
    expect(() => { (cmp as any).rollbackBlogVersion(0); }).not.toThrow();
  });
});
