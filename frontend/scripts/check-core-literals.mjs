 ...
 * reviewed allowlist. A NEW bare (non-`dark:`) core class or a new un-allowlisted hex in
 * these files turns CI RED — so WU0's surface-coverage enumeration is machine-checked,
 * not trusted.
 *
...
function scan(source) {
  ...
  for (let m = TW_CLASS.exec(source); m !== null; m = TW_CLASS.exec(source)) {
    ...
    found.push({ ...positionAt(source, m.index), text: m[0], kind: 'tw-class' });
  }
  HEX.lastIndex = 0;
  for (let m = HEX.exec(source); m !== null; m = HEX.exec(source)) {
    found.push({ ...positionAt(source, m.index), text: m[0], kind: 'hex' });
  }
  return found;
}
