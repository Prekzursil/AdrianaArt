...
* ``POST /theme/publish`` — atomic publish (promote draft → live) with the
  ``expected_version`` staleness 409 guard and the server-side contrast 422 gate
  over the EFFECTIVE default-merged set (B9).
* ``POST /theme/rollback/{version}`` — wholesale restore of a PUBLISHED snapshot
  (404 on a draft / forged / out-of-range version; contrast re-gated).
* ``POST /theme/reset-to-default`` — force-publish the seeded compiled defaults,
  audited, bypassing the staleness 409 (CB2).

Includes the two negative security tests for the WU4b rework — a partial-draft
publish that omits a pairing endpoint, and a rollback to a never-published draft
version — both of which pass the old subset-only / no-status-filter logic and are
rejected by the fixed effective-set gate.
...
# Individually-valid triplets whose text-on-background pairing FAILS AA (B9) — with
# BOTH endpoints present, so the old subset-only gate already caught it on publish.
CONTRAST_BYPASS_DRAFT = {
    "--background": "255 255 255",
    "--text": "200 200 200",
}
# The rework exploit: a light-grey --text with the --background endpoint OMITTED.
# WU2-valid in isolation; under the old subset-only gate the text-on-background
# pairing is SKIPPED (no --background in the subset) and ~1.6:1 grey-on-white ships.
...
def _insert_published_version(
    session_factory: async_sessionmaker, tokens: dict, version: int
) -> None:
    """Directly insert a PUBLISHED snapshot, bypassing the publish gate.

    Simulates a snapshot that predates the contrast gate (or was tampered with at
    rest) so the rollback-path contrast re-gate can be exercised end-to-end.
    """
...
def test_publish_partial_draft_contrast_bypass_is_rejected(
    seeded_app: Dict[str, object],
) -> None:
    """REWORK NEGATIVE #1 — a partial draft that omits a pairing endpoint.

    ``{"--text": "200 200 200"}`` is WU2-valid in isolation and, because it OMITS
    ``--background``, the old subset-only gate SKIPPED the text-on-background
    pairing and let ~1.6:1 grey-on-white ship live. The effective-set gate merges
    the compiled default (``--background`` = white) so the pairing is evaluated and
    the publish is rejected 422 — nothing reaches the storefront.
    """
    client: TestClient = seeded_app["client"]  # type: ignore[assignment]
    factory = seeded_app["session_factory"]  # type: ignore[assignment]
    token = _create_admin_token(factory)
    ...
    saved = client.put(
        "/api/v1/theme/draft",
        json={"tokens": PARTIAL_CONTRAST_BYPASS_DRAFT},
        headers=headers,
    )
    assert saved.status_code == 200, saved.text
    resp = client.post("/api/v1/theme/publish", json={}, headers=headers)
    ...
    assert "text-on-background" in failing_ids
    # The live storefront never shipped the failing grey — still the seeded default.
...
def test_rollback_to_draft_version_is_rejected(seeded_app: Dict[str, object]) -> None:
    """REWORK NEGATIVE #2 — rolling back to a never-published DRAFT version.

    A saved-but-unpublished draft is version 2 with ``status == draft``. The old
    rollback selected purely by ``version`` (no status filter) and force-published
    the ungated draft, sending it live WITHOUT the contrast gate. The fix restricts
    rollback to published snapshots, so a draft version is a hard 404 and the live
    storefront is untouched — no rollback audit, no promotion.
    """
    client: TestClient = seeded_app["client"]  # type: ignore[assignment]
    factory = seeded_app["session_factory"]  # type: ignore[assignment]
    token = _create_admin_token(factory)
    ...
    # Save a contrast-failing draft (version 2) but NEVER publish it.
    saved = client.put(
        "/api/v1/theme/draft",
        json={"tokens": PARTIAL_CONTRAST_BYPASS_DRAFT},
        headers=headers,
    ...
    assert saved.status_code == 200, saved.text
    assert _count(factory, ThemeVersion, status=ThemeStatus.draft, version=2) == 1

    resp = client.post("/api/v1/theme/rollback/2", json={}, headers=headers)
    assert resp.status_code == 404, resp.text
    # The ungated draft never went live and no rollback was recorded.
    assert _published(client)["tokens"] == default_theme_tokens()
    assert _count(factory, ThemeAuditLog, action="rollback:2") == 0


def test_rollback_to_failing_published_snapshot_is_rejected(
    seeded_app: Dict[str, object],
) -> None:
    """Defence-in-depth: a PUBLISHED snapshot that fails contrast is re-gated.

    Directly inserts a published snapshot whose ``--text`` is ~1.6:1 grey-on-white
    (a snapshot that predates the gate, or was tampered with at rest). Rollback
    re-runs the effective-set gate and refuses to ship it (422); the live
    storefront stays on the safe default.
    """
    client: TestClient = seeded_app["client"]  # type: ignore[assignment]
    factory = seeded_app["session_factory"]  # type: ignore[assignment]
    token = _create_admin_token(factory)
    ...
    _insert_published_version(factory, {"--text": "200 200 200"}, version=2)
    resp = client.post("/api/v1/theme/rollback/2", json={}, headers=headers)
    assert resp.status_code == 422, resp.text
    failing_ids = {f["pairing"] for f in resp.json()["detail"]["failures"]}
    assert "text-on-background" in failing_ids
...
# --------------------------------------------------------------------------- #
# Base-consistency: compiled_defaults() is the complete, registry-valid set
# --------------------------------------------------------------------------- #
def test_compiled_defaults_is_canonical_and_registry_valid() -> None:
    defaults = compiled_defaults()
    # compiled_defaults() is the canonical seed set.
    assert defaults == default_theme_tokens()
    # The spurious --shadow-color is gone; fonts carry the full curated stack.
    assert "--shadow-color" not in defaults
    assert defaults["--font-body"] == "Inter, system-ui, -apple-system, sans-serif"
    assert defaults["--font-heading"] == "Cinzel, ui-serif, Georgia, serif"
    # Every default value passes the WU2 validator (canonical == registry-valid),
    # which is what makes the effective-set contrast merge sound.
    for name, value in defaults.items():
        assert validate_token(name, value).ok, name
    # Every pairing endpoint is present, so the effective-set gate never skips.
    for pair in theme_contrast.PAIRINGS:
        assert pair.foreground in defaults, pair.foreground
